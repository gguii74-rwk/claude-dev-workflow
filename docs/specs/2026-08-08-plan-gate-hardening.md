...
- **보강(impl 루프 R2, fp-I2 — 판정 기준을 파일 존재에서 내용으로)**: fp-I1의 분기를 "ledger 파일 부재"로 잡으면 **실제 SDD 재개 순서에서 발동하지 않는다** — SDD 6.2.0 셋업은 plan을 읽기 전에 workspace를 만들고 identity-only `progress.md`를 생성하므로(SKILL.md 셋업 절), `git clean -fdx`로 유실된 뒤에도 수렴 검사 시점에는 ledger가 "존재"한다. 따라서 판정 기준을 **내용**으로 옮긴다: ledger가 없거나, 있어도 **어느 task의 완료 기록도 없는데 표에는 이미 `[x]` 행이 있으면** 그 ledger는 fresh 또는 유실이라 권위가 아니다 → 커밋된 표로 폴백(일괄 되돌림 금지). 폴백 후에는 **살아남은 표로 ledger의 완료 줄을 복원**해 이후 검사가 다시 live ledger를 갖게 한다(복원 범위는 fp-I4가 한정한다). R1 수정 자신의 회귀 교정이다(spec 루프 fp-C6 전례 — 신규 경계 추격이 아님).
- **보강(impl 루프 R3, fp-I4 — 복원의 범위 한정, 사용자 판정 FIXED)**: R2가 넣은 "표로 ledger 완료 줄 복원"은 **완료 상태만** 되살린다. SDD ledger는 `Task N: minor (deferred)`·parked finding과 ruling도 보관하고 **final whole-branch review가 그 목록을 명시적으로 넘겨받도록** 규정돼 있어(SDD §fix loop·§final review — "A roll-up nobody reads is a silent discard"), 유실 경로에서 복원된 ledger는 live처럼 보이지만 리뷰 부채를 잃은 상태다. 따라서 계약 문안은 **복원이 완료 상태에 한정됨을 명시하고, final review 진입 전 그 공백을 사람에게 알려 확인받도록** 한다. 리뷰어 1안(parked/deferred를 entrypoint에 함께 보존)은 **비목표 "outcome 형식·내용 규정 강화"와 "SDD 수정" 경계에 저촉**해 채택하지 않았다 — 한 줄 outcome 규약·SDD 불변 유지.
- **보강(확인 C1 → 복귀 R4, fp-I1·fp-I6 — non-SDD 다리 철회, 사용자 판정)**: R1이 폴백의 발동 경로에 "SDD 없이 직접 실행"을 넣은 것이 **범위 확대였다**. D2 기결정의 표현은 "**subagent 없이** 직접 실행하면 실행자 자신"으로 SDD **안**에서의 직접 실행을 뜻하고, 계약 ④ 자신도 첫 문장에서 SDD를 `REQUIRED SUB-SKILL`로 선언한다 — 두 규정이 공존하면 계약이 자기모순이고(R4 지적), 실제로 non-SDD 경로는 **수렴 불가**를 낳았다(확인 C1 지적: 첫 완료는 표 폴백으로 ledger 복원 → 이후 ④는 표만 갱신 → 부분 ledger가 "live"로 판정돼 새 완료 행마다 되돌려짐). 따라서 **non-SDD 다리를 철회**한다(사용자 판정, batch 밖 즉시 ESCALATE). 폴백의 발동 경로는 **SDD를 쓰는 중 ledger가 fresh/유실인 경우**로 한정된다 — final review 후 `rm -rf <workspace>` cleanup, `git clean -fdx`, 그리고 셋업이 재생성한 identity-only ledger. 리뷰어 대안 2종(직접 실행자가 ledger 완료 줄까지 갱신 / 계약을 2모드로 분기하고 직접 모드의 완료 동기점을 신설)은 각각 SDD 없는 실행에 SDD 산물 유지를 요구하거나 D2 범위 밖 신규 규정을 만들어 미채택.
...
- **보강(impl 루프 R2, fp-I3 — 관문 ①의 검사 내용)**: ①은 블록 **존재**만 보면 구버전 계약(④ 완료 기록 절 없음)을 정상 통과시킨다 — base 커밋의 구버전 entrypoint가 ①②④를 모두 통과하는 것이 실증이다. 구버전 플러그인·미갱신 머신이 만든 plan이 이번 트랙의 핵심 보호(D2) 없이 승인되므로, ①의 검사 내용을 **"현행판인지"**까지로 명확히 한다. **판정 기준은 특정 절의 유무가 아니라 설치된 writing-plans-split의 canonical verbatim 블록과의 대조**다(복귀 R4, fp-I5) — 절 유무로 잡으면 그 절은 있으나 이후 보강이 빠진 **중간 버전**이 통과해 재개 시 승인 완료 task를 되돌린다. 스킬 문서가 원본이라 버전 표식·파서를 두지 않고 두 문면을 나란히 읽어 판단하므로 버전 중립적이다. **항목 수는 4종 그대로**(D3 재론 아님) — 다섯 번째 관문·파서 신설 없음(D7·G1·G2 준수). 실패 시 처리 = 블록 교체(저비용 → D5의 "해결 후 진행"), 대상은 이번 루프가 리뷰할 plan뿐이다(§2 비목표 "소급 적용 없음" 경계 유지).
...
- (a) writing-plans-split Execution contract에 완료 기록 의무 ④가 계약 문면으로 들어가고(D2 — 주체·시점(완료 확정 직후, fp-C3)·커밋·수렴 검사 3시점·3원 대조(완료 권위 = SDD ledger 완료 기록, git log는 보조 검증, 부재 시 fail-closed)(fp-C2·fp-C4·fp-C6) 포함), task 표 서술·§Execution handoff와 모순이 없다. 계약 문안의 수렴 검사는 미기입 행뿐 아니라 **기입-미커밋 상태**를 수렴 대상에 포함한다(fp-C5 DEFERRED_TO_IMPL — 구현 시 문안에 반영, §6 GREEN 시나리오로 검증). 또한 fail-closed 되돌림은 **live ledger를 전제**하고, ledger가 없거나 **완료 기록이 하나도 없는데 표에 `[x]`가 있는** fresh/유실 상태(cleanup·`git clean -fdx` 후 identity-only 재생성 — non-SDD 실행은 fp-I6으로 철회)에서는 커밋된 표를 권위로 폴백해 **일괄 되돌림을 금지**하고 표로 ledger를 복원하되, **복원 범위가 완료 상태에 한정됨을 명시하고 deferred/parked 공백은 final review 전 사람 확인으로 넘긴다**(fp-I1·fp-I2·fp-I4). §Execution handoff에는 SDD 6.2.0 task-brief 접점 어댑터 1줄이 들어간다(fp-C1 — §4 표).
- (b) review-loop plan 사전 게이트에 형식 관문 4종(D3)이 추가되고, 적용 조건(D1)·단일 task 예외(D1)·승계 ledger 부재 분기(D11)·실패 동작(D5)·명세 수위(D7)가 명시된다. 관문 ①은 블록 **존재**가 아니라 **설치된 writing-plans-split의 canonical verbatim 블록과 대조한 현행판 여부**까지 본다(fp-I3·fp-I5 — 항목 수는 4종 불변, 버전 표식·파서 없음).
...
| **fp-I4** — writing-plans-split 계약 ④ · ledger 폴백이 completion만 복원해 SDD의 deferred-minor·parked finding과 ruling이 조용히 폐기 — final review가 그 목록을 전달받도록 규정돼 있어, 완료 task는 건너뛰면서 알려진 결함만 영구 소실 · 권고 = finding까지 보존하거나, 확인 불가 시 재실행 없이 사람 확인으로 fail-closed | high (R3) | **ESCALATE → FIXED (사용자 판정, batch flush 2026-08-08)** | 사실 검증: SDD가 `Task N: minor (deferred)`·parked를 ledger에 적재하고 final review 디스패치가 그 목록을 가리키도록 규정("A roll-up nobody reads is a silent discard" — SDD §fix loop·§final review). **fp-I1 계열 3회째 + R3 판정 루프 국면**이라 자동 수정하지 않고 batch ESCALATE로 올림. 리뷰어 1안(entrypoint에 finding 보존)은 **비목표 "outcome 형식·내용 규정 강화"·"SDD 수정" 저촉**이라 선택지에서 제외. 사용자 채택 = 최소 문안(복원 범위가 완료 상태 한정임을 명시 + final review 진입 전 사람 확인) — 형식 확장·SDD 변경 없음. 판정 주체 = 사용자 |
...
**batch flush (2026-08-08, 적대 소진 3 = auto-rounds 도달 + 전환 신호 2 발화)**: ① batch ESCALATE 1건(fp-I4) 사용자 제시 → **FIXED(최소 문안)**으로 종결. ② **D9 재검증 범위** → 사용자 판정 = **릴리스 전 별도 세션으로 이월**(impl 루프는 종결하고, 0.10.0 릴리스 전 새 세션에서 신규 시나리오 GREEN 실행 — 핸드오프 blocking 체크). ③ 자동 수정 2건(R1 `045b9cf` · R2 `e5e3977`) **전부 승인**. → 확인 모드 진입(확인 필수 조건 성립: 미확인 FIXED 큐 4건, 확인 예산 2/2 잔여).
