...
- **보강(확인 C1, fp-I1 잔존 해소 — SDD 유실과 SDD 부재의 분리)**: R1~R3 누적 문안이 "ledger 유실"과 "SDD 자체를 안 씀"을 한 분기로 묶어, non-SDD 직접 실행에서 **수렴 불가**가 생겼다 — 첫 완료는 표 폴백으로 ledger가 복원되지만 ④는 표만 갱신하므로 부분 ledger가 "live"로 판정돼 이후 완료 행마다 `[ ]`로 되돌아간다. 따라서 둘을 분리한다: **ledger 복원은 SDD를 쓰는 경우에만** 하고, **SDD를 쓰지 않으면 ledger 조항 전체가 그 실행 내내 적용되지 않으며 커밋된 표가 단독 권위**다(애초에 쓸 일 없는 ledger 기록의 부재로 행을 되돌리지 말 것 + 반쪽 ledger를 남기지 말 것). 리뷰어 대안(직접 실행자가 ledger 완료 줄까지 갱신)은 SDD 없는 실행에 SDD 산물 유지를 요구해 범위·취지에 어긋나 미채택.
...
**미확인 FIXED 큐**: **fp-I1** (확인 C1에서 blocking 재분류 → R4 재수정, 재확인 대상). fp-I2·fp-I3·fp-I4는 확인 C1에서 소멸 확인(큐 제거).
...
| R4 | 적대(복귀, 상한 밖 — 소진 3 유지) | (아래 기록) | fp-I1 재수정분의 회귀 확인 라운드 | 1 |
