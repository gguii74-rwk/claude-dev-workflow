...
- 오르카: reason에 사전 검증(`terminal show`)·create/wait/send 명령 · 옛 세션 종료 대기 `--for exit`(`tui-idle` 불인정) · 폴더 잡 조회 = `state.mjs` `loadState`(`status --all` 불사용) · `ORCA_TERMINAL_HANDLE` 값 · 제목 정규화 규칙(`[A-Za-z0-9._-]`)·토큰과 `terminal list` 회수 · 프롬프트 파일 + `"$(cat …)"` 전달 문구 · `turn_started` 확인과 `--retry-request` 재관찰 · 첫 동작 세 명령 전부에 `--terminal <옛 핸들>`과 단계별 성공 조건·실패 시 차단(R5-2) · 종료 전 폴더 잡 확인·대기(R5-3) · 폴백 조건 4종과 정리 방식이 있고, "자가 /clear는 불가" 문장이 없다. 재개 프롬프트 템플릿 안에 `--terminal` 없는 send/wait/close가 0건.
