...
- 새 (2) 골격(오르카 경로):
  0. **자동 인계 사전 조건**(R6-2): codex 플러그인의 Stop review gate가 켜져 있으면 자동 인계를 하지 않고 F2(현행 `/clear` 안내). 조회는 **상태 파일(`resolveStateFile(cwd)`)을 직접 한 번 읽어 `JSON.parse`한 객체의 `config.stopReviewGate`** — `loadState`는 읽기·파싱 실패를 기본값(`false`)으로 돌려주므로 쓰지 않는다(C2 행 4). **파일 부재는 "게이트 꺼짐·잡 없음"으로 읽되, 읽기·파싱 실패는 F2** — 그 게이트는 `stop_hook_active`를 보지 않고 동기 codex 작업 뒤 block을 돌려줄 수 있어, 후계의 `/exit`가 그 턴을 끊는다. 게이트가 꺼져 있을 때만(기본값·이 저장소 설정) 아래로 진행. **CLI 해소·사전 검증**(R1-1): 실행 파일 = orca-cli 스킬 순서 — `ORCA_CLI_COMMAND`가 있으면 그 값, 없고 `ORCA_DEV_REPO_ROOT`가 있으면 `orca-dev`, 그 외 `orca`. 이어서 `<CLI> terminal show --terminal $ORCA_TERMINAL_HANDLE --json`이 `ok:true`가 아니면 그 CLI는 현재 인스턴스가 아니다 — 후계를 만들지 않고 F2.
  1. `<CLI> terminal create --worktree active --title "<작업명>-<토큰>" --command claude --json`(1.0.2: 1.0.1의 환경변수 접두 `CLAUDE_CODE_DISABLE_TERMINAL_TITLE=1`은 오르카 tui-idle 판정용 탭 제목 쓰기까지 막아 2.의 기동 대기가 항상 시간 초과 — 제거, D2 그대로. plan AC6 2회차 비고) → 새 핸들 = `startupTerminal.handle`(없으면 `terminal.handle`). **토큰** = 옛 세션이 만든 고유 상관 문자열(예: `HHMMSS` + 난수 4자, 영숫자·하이픈만)(R2-3). **제목 전체는 안전 문자 집합으로 정규화한다**(R3-1): `[A-Za-z0-9._-]` 외 문자는 `-`로 치환, 제목 전체 길이 ≤ 40 — **`-<토큰>` 길이를 먼저 예약하고 작업명 부분만 절단한 뒤 토큰을 온전히 붙인다**(R4-1: 토큰이 잘리면 탭에서 후계를 식별할 상관 문자열이 사라진다). 제목은 셸 큰따옴표 안에 들어가므로 `$()`·백틱·따옴표·공백을 남기지 않는다(D7의 "짧은 작업명"은 이 정규화를 거친 값). 응답·JSON이 유실되면 재생성하지 말고 `<CLI> terminal list --worktree active --json`을 다시 실행해 **create 직전에 기록한 핸들 집합에 없는 새 핸들**을 후보로 모아 **채택하지 않고** 차단 보고한다 — 오르카에는 생성 요청과 터미널을 묶는 불변 식별자가 없어 후보가 1개여도 그 사이 사용자·다른 세션이 연 탭일 수 있다(plan impl G1-1). create 직전 목록 조회가 실패하거나 `truncated:true`면 후계를 만들지 않고 F2(1.0.2: 탭 제목은 셸 PS1·claude가 곧 덮어써 제목 조회 회수가 성립하지 않는다 — plan AC6 2회차 비고 원인 B).
  ...
  4. 재개 프롬프트(훅이 템플릿 제공) = "핸드오프 파일 <경로>를 읽고 같은 작업을 이어서 진행. **0번째 동작 — 공유 브로커 단일 실행권 검사**(R5-3, 사용자 확정): `/exit` 전에 이 폴더의 codex 상태 파일을 직접 읽어 `queued`/`running` 잡 중 **자기 세션 외**(`sessionId ≠ CODEX_COMPANION_SESSION_ID`)의 것을 확인한다 — `codex-companion.mjs status --all`은 **자기 세션 잡만 필터링**하므로 쓰지 않는다(C1-#11, job-control.mjs). 조회 = `node -e` 한 줄로 **상태 파일을 직접 한 번 읽어 `JSON.parse`한 동일 객체의 `jobs`**를 본다(파일 위치만 `<companion 루트>/scripts/lib/state.mjs`의 `resolveStateFile(cwd)`로 얻는다 — cwd는 repo 루트; companion 루트는 RL §2b ①과 같은 레지스트리 해소). `loadState(cwd)`로 다시 읽지 않는다 — 검증과 소비 사이에 파일이 바뀌면 파싱 실패가 빈 `jobs`로 돌아온다(C2 행 3). **읽기·파싱 실패는 "잡 없음"이 아니라 차단이다**(R6-1a: 사용자 보고·보류). 검사와 종료 사이의 수초 경쟁 창은 수용한다(ledger ACCEPTED, 사용자 재판정 — 보완 없음, 후속 = drain/lease). 있으면 완료까지 15초 간격으로 기다리고(상한 10분), 상한에 닿으면 사용자에게 보고하고 `/exit`·codex 시작을 모두 보류한다 — 옛 세션의 SessionEnd가 폴더 공유 브로커를 내려 그 잡을 죽인다. 브로커 참조 계수화는 codex 플러그인 소관이라 범위 밖(후속). **첫 동작**(R2-1 — 세 명령 모두 옛 핸들에 바인딩, `--terminal` 생략 금지 — 생략하면 활성 터미널이 대상이 되어 후계 자신이나 무관한 탭을 닫는다): `<CLI> terminal send --terminal <옛 핸들> --text "/exit" --enter --json` → `<CLI> terminal wait --terminal <옛 핸들> --for exit --timeout-ms 30000 --json`(**claude 프로세스 종료** 대기 — `tui-idle`은 옛 세션이 이미 idle이라 즉시 만족돼 증거가 못 된다, C1-#10) → `<CLI> terminal close --terminal <옛 핸들> --json` (CLI 해소는 0단계와 같은 순서). **각 단계의 성공 조건**(R5-2·C1-#10): `/exit` send 영수증 `accepted:true` → `--for exit` wait `satisfied:true`, 그것이 불가한 호스트면 `terminal read`의 마지막 줄들에 claude 종료 표지("Resume this session with `claude --resume …`")와 셸 프롬프트 복귀가 **둘 다** 보여야 한다(`tui-idle` 불인정) → close `ok:true` → `terminal list`에 옛 핸들 부재. 어느 단계가 실패하거나 모호하면(예: send 실패인데 옛 세션이 이미 idle이라 wait만 통과) **close도 codex 시작도 하지 말고** 상태를 사용자에게 보고하라(close만 하면 SessionEnd가 건너뛰어져 브로커·잡이 고아가 된다). 그 전에는 codex를 실행하지 마라. 옛 핸들이 `terminal list`에 없으면 이미 닫힌 것으로 보고 진행하고, 있는데 닫기가 실패하면 사용자에게 알리고 codex 라운드를 시작하지 마라(D9). **핸드오프의 다음 액션이 다른 단계(spec→plan, plan→impl)의 시작이면 시작하지 말고 사용자에게 확인하라**(D4)." 경로 = 핸드오프 규약 파일(기본 `.remember/remember.md`, review-loop면 루프 파일). `--title` = 모델이 현재 작업을 요약한 짧은 이름, 마땅치 않으면 `successor`(D7).
...
폴백 조건 4종: (a) 사전 검증 실패(F1 0) · (b) wait `satisfied:false`(재시도 후) · (c) `turn_started` 미확인(재관찰 후) · (d) CLI 실행 오류(재조정 후 — create 응답 유실은 회수하지 않는다 — 생성 전후 핸들 집합 차분으로 후보만 산출해 **재생성과 `/clear` 안내를 모두 차단**하고 후보 핸들을 사용자에게 보고한 뒤 정지한다(새 핸들을 모르므로 반쪽 터미널 정리도 하지 않는다), R2-3·1.0.2 G1-1). **미전달이 확정되기 전(재관찰·재조정 중)에는 후계를 닫지 않는다**(R1-2 — 후계가 이미 프롬프트를 받아 옛 세션에 `/exit`를 보내는 중일 수 있다). 확정되면 후계를 만들지 못한 것으로 보고 현행 문장("이어서 진행하려면 /clear 후 같은 작업을 다시 시작하세요")을 사용자에게 안내한다. 만들다 만 터미널은 옛 세션이 폴백 안내 전에 정리한다(D10 — 정리 주체). **정리 방식**(R1-3): claude가 뜬 이력이 있으면(wait `satisfied:true`였거나 `terminal read`에 claude 프롬프트가 보이면) `/exit` 전송 → `--for exit` 대기(또는 `terminal read`의 종료 표지 + 셸 프롬프트 — `tui-idle` 불인정, C1-#3) → `terminal close`; 뜨지 않았으면 `terminal close` 직접. `/exit` 처리 증거 없이 close하지 않는다(증거가 없으면 사용자에게 보고). 어느 쪽이든 `terminal list`로 소멸을 확인하고, 실패하면 `/clear` 안내 전에 차단 상태(핸들·원인)를 사용자에게 보고한다. fail-closed.
...
- 오르카: reason에 사전 검증(`terminal show`)·create/wait/send 명령 · 옛 세션 종료 대기 `--for exit`(`tui-idle` 불인정) · 폴더 잡·게이트 조회 = 상태 파일 직접 1회 파싱한 동일 객체(`loadState` 불사용·`status --all` 불사용)·파싱 실패 = 차단/F2(R6-1a·R6-2·C2) · review gate 켜짐 = F2(R6-2) · `ORCA_TERMINAL_HANDLE` 값 · 제목 정규화 규칙(`[A-Za-z0-9._-]`)·토큰과 `terminal list` 회수 · 프롬프트 파일 + `"$(cat …)"` 전달 문구 · `turn_started` 확인과 `--retry-request` 재관찰 · 첫 동작 세 명령 전부에 `--terminal <옛 핸들>`과 단계별 성공 조건·실패 시 차단(R5-2) · 종료 전 폴더 잡 확인·대기(R5-3) · 폴백 조건 4종과 정리 방식이 있고, "자가 /clear는 불가" 문장이 없다. 재개 프롬프트 템플릿 안에 `--terminal` 없는 send/wait/close가 0건.
- `stopHookActive`·구간 로직은 기존 동작 불변(기존 케이스 유지).
- 제목 절단: 40자를 넘는 작업명에서도 제목 끝의 토큰이 온전히 남는 케이스(R4-1).
...
- **AC2 (F2)**: 오르카 reason에 CLI 해소 순서와 사전 검증(R1-1) · 제목 상관 토큰·응답 유실 시 생성 전후 `terminal list` 핸들 집합 차분 후보 보고·채택 금지(1.0.2)·미확정 시 재생성/안내 차단(R2-3) · 프롬프트 파일 전달·보간 금지(R2-2) · 제목 안전 문자 집합 정규화(R3-1)·토큰 길이 예약 후 작업명만 절단(R4-1) · 첫 동작 세 명령의 `--terminal <옛 핸들>` 바인딩(R2-1) · `turn_started` 확인·`--retry-request` 재관찰·미전달 확정 전 후계 유지(R1-2) · 폴백 조건 4종 · 반쪽 터미널 정리 방식(`/exit`→close 또는 직접 close)과 `terminal list` 소멸 확인·실패 시 차단 보고(R1-3) · 복귀 문장이 명시된다. 재개 프롬프트 템플릿에 단계 경계 확인(D4)·닫기 실패 처리(D9)·탭 제목 규칙(D7)이 들어간다.
...
- **범위 밖(후속)** codex 플러그인의 폴더 공유 브로커를 참조 계수 방식으로 바꾸는 것(R5-3 권고 후반)과 활성 잡 검사·종료권 이전을 원자화하는 drain/lease API(R6-1 권고) — 외부 플러그인 소관. 이 트랙은 종료 전 잡 확인·대기 + 파싱 실패 차단으로 대응.
- **ACCEPTED(사용자 재판정 2026-09-24, R6-1b)** 잡 검사와 SessionEnd 사이 수초의 경쟁 창에 제3 세션이 새 잡을 시작하면 그 잡이 끊긴다 — 같은 폴더 동시 세션은 규약상 드물고 현행 사람 `/clear`와 같은 위험. **보완 없음**(F6은 정상 인계 1회만 본다), 후속 = 위 drain/lease.
...
| spec:F3 · 사용자의 명시적 중단 선택도 자동 재개된다 · §2i 조건화를 경로 ①에만 | high | FIXED `b4ced82` | F3 경로 ①(넛지)에만 조건화, 경로 ②·③ 현행 유지. AC3 세 경로 기대 동작 명시 |
| spec:F1 · `/exit` 전달 실패 뒤에도 강제 종료로 진행 · 단계별 성공 조건 + 실패 시 close/codex 차단 | high | FIXED `b4ced82` | F1 (4) 첫 동작 단계별 성공 조건(영수증·satisfied/셸 복귀·ok·list 부재)과 차단·보고. AC2 |
