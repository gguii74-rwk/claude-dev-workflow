...
- 새 (2) 골격(오르카 경로):
  ...
  4. 재개 프롬프트(훅이 템플릿 제공) = "핸드오프 파일 <경로>를 읽고 같은 작업을 이어서 진행. **0번째 동작 — 공유 브로커 단일 실행권 검사**(R5-3, 사용자 확정): `/exit` 전에 이 폴더의 codex 상태 파일을 직접 읽어 `queued`/`running` 잡 중 **자기 세션 외**(`sessionId ≠ CODEX_COMPANION_SESSION_ID`)의 것을 확인한다 — `codex-companion.mjs status --all`은 **자기 세션 잡만 필터링**하므로 쓰지 않는다(C1-#11, job-control.mjs). 조회 = `node -e` 한 줄로 `<companion 루트>/scripts/lib/state.mjs`의 `loadState(cwd).jobs`를 읽는다(상태 파일 위치는 `resolveStateFile(cwd)` — cwd는 repo 루트; companion 루트는 RL §2b ①과 같은 레지스트리 해소). **읽기·파싱 실패는 "잡 없음"이 아니라 차단이다**(R6-1: `loadState`는 파싱 실패를 빈 상태로 돌려주므로 상태 파일을 먼저 `JSON.parse`로 직접 읽어 실패 시 사용자 보고·보류). 검사와 종료 사이의 수초 경쟁 창은 수용한다(ledger ACCEPTED — 보완 = F6 관찰, drain/lease는 codex 플러그인 후속). 있으면 완료까지 15초 간격으로 기다리고(상한 10분), 상한에 닿으면 사용자에게 보고하고 `/exit`·codex 시작을 모두 보류한다 — 옛 세션의 SessionEnd가 폴더 공유 브로커를 내려 그 잡을 죽인다. 브로커 참조 계수화는 codex 플러그인 소관이라 범위 밖(후속). **첫 동작**(R2-1 — 세 명령 모두 옛 핸들에 바인딩, `--terminal` 생략 금지 — 생략하면 활성 터미널이 대상이 되어 후계 자신이나 무관한 탭을 닫는다): `<CLI> terminal send --terminal <옛 핸들> --text "/exit" --enter --json` → `<CLI> terminal wait --terminal <옛 핸들> --for exit --timeout-ms 30000 --json`(**claude 프로세스 종료** 대기 — `tui-idle`은 옛 세션이 이미 idle이라 즉시 만족돼 증거가 못 된다, C1-#10) → `<CLI> terminal close --terminal <옛 핸들> --json` (CLI 해소는 0단계와 같은 순서). **각 단계의 성공 조건**(R5-2·C1-#10): `/exit` send 영수증 `accepted:true` → `--for exit` wait `satisfied:true`, 그것이 불가한 호스트면 `terminal read`의 마지막 줄들에 claude 종료 표지("Resume this session with `claude --resume …`")와 셸 프롬프트 복귀가 **둘 다** 보여야 한다(`tui-idle` 불인정) → close `ok:true` → `terminal list`에 옛 핸들 부재. 어느 단계가 실패하거나 모호하면(예: send 실패인데 옛 세션이 이미 idle이라 wait만 통과) **close도 codex 시작도 하지 말고** 상태를 사용자에게 보고하라(close만 하면 SessionEnd가 건너뛰어져 브로커·잡이 고아가 된다). 그 전에는 codex를 실행하지 마라. 옛 핸들이 `terminal list`에 없으면 이미 닫힌 것으로 보고 진행하고, 있는데 닫기가 실패하면 사용자에게 알리고 codex 라운드를 시작하지 마라(D9). **핸드오프의 다음 액션이 다른 단계(spec→plan, plan→impl)의 시작이면 시작하지 말고 사용자에게 확인하라**(D4)." 경로 = 핸드오프 규약 파일(기본 `.remember/remember.md`, review-loop면 루프 파일). `--title` = 모델이 현재 작업을 요약한 짧은 이름, 마땅치 않으면 `successor`(D7).
...
- 오르카: reason에 사전 검증(`terminal show`)·create/wait/send 명령 · 옛 세션 종료 대기 `--for exit`(`tui-idle` 불인정) · 폴더 잡 조회 = `state.mjs` `loadState`(`status --all` 불사용)·파싱 실패 = 차단(R6-1) · review gate 켜짐 = F2(R6-2) · `ORCA_TERMINAL_HANDLE` 값 · 제목 정규화 규칙(`[A-Za-z0-9._-]`)·토큰과 `terminal list` 회수 · 프롬프트 파일 + `"$(cat …)"` 전달 문구 · `turn_started` 확인과 `--retry-request` 재관찰 · 첫 동작 세 명령 전부에 `--terminal <옛 핸들>`과 단계별 성공 조건·실패 시 차단(R5-2) · 종료 전 폴더 잡 확인·대기(R5-3) · 폴백 조건 4종과 정리 방식이 있고, "자가 /clear는 불가" 문장이 없다. 재개 프롬프트 템플릿 안에 `--terminal` 없는 send/wait/close가 0건.
...
- **ACCEPTED(루프 판정, R6-1)** 잡 검사와 SessionEnd 사이 수초의 경쟁 창에 제3 세션이 새 잡을 시작하면 그 잡이 끊긴다 — 같은 폴더 동시 세션은 규약상 드물고 현행 사람 `/clear`와 같은 위험. 보완 = F6 실사용 관찰, 후속 = 위 drain/lease.
