...
- **C-4(핸드오프 표준화)는 7a에서 분리해 별도 트랙으로 이월 (2026-08-09 사용자 판정).** 근거는 실측 — 7a spec 루프 finding **10건 중 6건이 C-4**에서 나왔고 C-3·C-7은 4라운드 적대에서 0건이었다. 설계 원문과 잔여 blocking 4건은 `2026-08-09-ledger-handoff-mockup-nudge.md` §부록에 보존돼 있다. **배치 순번 미정** — 7b·순번 8과의 우선순위는 사용자 결정 사항이다.
