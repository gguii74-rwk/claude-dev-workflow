...
| 7a | 소폭 묶음: impl ledger 위치 명시 · ui-mockup 2건(생략 D번호+근거·슬러그 접미) · 훅 재넛지 | C-3·C-7·C-8 | review-loop·ui-mockup·훅 |
| 7b | doctor 점검 신설 + setup 마커 문구 + dev-cycle description | C-9 (+ C-6 D11 · fp-I3) | doctor(신설)·setup·dev-cycle |
| 7c | 핸드오프 표준화 (4섹션 표준 승격 + 전용 파일 분리) | **C-4** | review-loop |
...
- **순번 7 = 7a/7b 분할 (2026-08-09 사용자 결정).** 근거는 **검증 하네스 계열이 다르다**는 것 — 7a(C-3·C-7)는 C-6 dcx 하네스를 재사용하는 절차 판별 arm이고 C-8은 모델 예산을 쓰지 않는 순수 함수다. 반면 7b(C-9)는 신규 스킬 description의 **발견성 TDD**를 요구해 하네스를 새로 만들어야 한다. 한 트랙에 얹으면 C-6 실측(2항목 흡수 = arm 29종·243런)이 경고한 구간에 들어간다.
- **C-4(핸드오프 표준화)는 7a에서 분리해 별도 트랙으로 이월 (2026-08-09 사용자 판정).** 근거는 실측 — 7a spec 루프 finding **10건 중 6건이 C-4**에서 나왔고 C-3·C-7은 4라운드 적대에서 0건이었다. 설계 원문과 잔여 blocking 4건은 `2026-08-09-ledger-handoff-mockup-nudge.md` §부록에 보존돼 있다. **배치 = 순번 7c (2026-08-09 사용자 결정)** — 7b 다음, 순번 8 앞. 근거 ① 순번 7(7a+7b)을 먼저 닫아 1~8 순차 원칙을 유지한다 ② C-4는 blocking 4건이 남아 **1–2단계(brainstorming)부터** 시작하는 트랙이라 범위가 이미 확정된 7b보다 착수 비용이 크다 ③ 순번 8(C-10) **앞**에 두어야 개선된 핸드오프가 남은 트랙에서 실제로 쓰인다.
