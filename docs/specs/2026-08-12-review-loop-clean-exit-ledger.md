...
- **부작용 실측 어댑터**: harness-7c-slim의 read-only 서술 규약으로는 AC1의 부작용(종결 1줄 기록·§3 커밋 경유)을 실측할 수 없다 — 부작용 검증이 필요한 케이스(P 전건 + R 중 기록 동작 검증 케이스)는 **픽스처 repo의 writable 임시 clone**에서 실행하고, 채점은 응답 서술이 아니라 **사후 git oracle**로 한다: `git show HEAD:<규정 문서>`로 종결 1줄 실재·커밋 경유를 확인하고, 필드가 픽스처 실측치와 정합(라운드 수·low 수·base/target SHA 일치, placeholder 잔존 = RED)해야 GREEN. D13(하네스 재사용)은 유지 — 실행 모드 추가이지 하네스 교체가 아니다.
...
## 트랙 종결 (2026-08-12) — 7단계 TDD RED 부정 결과, SKILL 무변경 종료

- **RED 실측(65런, 폐기 0)**: AC1의 전제 "빠른 종료 시 종결 기록 부재"가 **미재현** — P1~P4·RLOW 25/25 기록 실재(자유 형식 11 + 규격줄 존재·필드 상이 14, 부재 0) → §5 결함 미재현 분기(배치 중단·재설계 논의) 발동. G2 이탈 0(자연 수렴 5/5). 반면 기준선은 전건 정합: G1·G3·G4 현행 분기 5/5, RNC 행위 정합 5/5, C6·C8 norec 교착 재현 5/5(C4는 교착 재현·지목 4런이 3.5로 분산 — "배너 슬롯" 픽스처 경계 교란 관측).
- **판독 프로브(사용자 선택 분기, 5런)**: RC6-free = RC6-norec + cur이 실제 남긴 최약 자유형식 plan ledger(규격줄·target SHA 없음, 커밋 `dd952ac`) → C6 5/5 "7 구현" 지목. **자유형식 기록도 dev-cycle이 판독한다** — 형식 표준화의 소비자(AC2) 실익 측정상 0. 교착은 "기록 전무" 상태에서만 발생하며 그 상태는 현행 생산자 행동(25/25 기록)으로는 도달하지 않는다.
- **결정(사용자 판정 2026-08-12)**: 가설 2건(기록 부재·자유형식 판독 불가) 기각 → **SKILL.md 무변경 · GREEN 78런 취소 · 트랙 종료**. 부정 결과도 산출물(LESSONS L1 "성장은 실측 뒤에"·L5, H3 기각 방향과 일치).
- 실측 원본: `.remember/harness-7c/scores-i17.md`(RED+PROBE 채점) · `oracle-cur-red.txt` · `out-i17/`·`work-i17/`(런 원문·작업 사본 감사 보존) · `.remember/harness-8/out-i17/`. 사용량 ≈ 서브에이전트 4.74M tok. 하네스 결정적 소스의 repo 커밋(`docs/specs/harness-i17/` 후보)은 impl 단계 부재로 유예 — 필요 시 후속 판단.
