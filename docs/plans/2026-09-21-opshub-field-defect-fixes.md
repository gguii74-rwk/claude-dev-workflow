...
기준 61,898B → 상한 **69,066B**(`wc -c dev-workflow/skills/review-loop/SKILL.md`). **plan 합성 실측**(task-02~05 교체문을 0.17.0 RL에 그대로 적용, review-loop(plan) R1): task-02 후 64,622 · task-03 후 65,894 · task-04 후 66,864 · task-05 후 68,610(R2~R5 수정 반영, 여유 456B). 소프트 예산(합성값 + ≈130B): task-02 후 ≤ 64,750 · task-03 후 ≤ 66,000 · task-04 후 ≤ 67,000 · task-05 후 ≤ 68,750. 하드 확인 = task-06(초과 시 task-06 §압축 후보에서 줄인다 — 상한을 다시 올리지 않는다). 소프트 예산을 넘으면 교체문을 그대로 붙이지 않은 것이므로 먼저 diff로 원인을 찾는다.
...
| 02 | RL 실행 절차 재작성 — F1·F2 | [x] | [task-02](2026-09-21-opshub-field-defect-fixes/task-02-review-loop-execution.md) | 01 | 2026-09-22 `155876c` — §A·§B 교체(브리프 바이트 동일), RL:156·:285 삭제, 보존행 158·282·286~288 무변경, AC 전 항목 일치, 64,622B(합성값 동일, 소프트 예산 64,750 이내), 리뷰 clean(minor 2 deferred: SC-4 RESOLVE_FAIL 비정지·`$L` 산문 정의) |
| 03 | RL 유효성 블록 + focus 고정 줄 — F3 | [x] | [task-03](2026-09-21-opshub-field-defect-fixes/task-03-review-loop-validity-focus.md) | 02 | 2026-09-22 `9dd81f3` — ④ 유효성 블록·빈 가드 정밀화·고정 첫 줄 불릿(RL:110, D5 짝 앞) 3곳 바이트 동일, FOCUS-LINE 271B = SC-3, AC 전 항목 일치, 65,894B(합성값 동일), 리뷰 clean(minor 3 deferred) |
| 04 | RL ledger 순서 규정·소규모 정정 — F5·F7(RL:337)·F8 | [x] | [task-04](2026-09-21-opshub-field-defect-fixes/task-04-review-loop-ledger-misc.md) | 03 | 2026-09-22 `8df3637` — 6곳 1행 교체(83 F5·246 F8c·262 F8a·330 F8d·331 F8b·353 F7) 바이트 동일, `40%` 0·§4 grep 불변, AC 전 항목 일치, 66,864B(합성값 동일), 리뷰 clean(minor deferred: 순서 규정 vs §3 "§2a 커밋" 문구 해석) |
| 05 | 넛지 훅 문구 + RL §2i 진행 중 라운드 분기 — F6 | [x] | [task-05](2026-09-21-opshub-field-defect-fixes/task-05-nudge-hook-resume.md) | 04 | 2026-09-22 `edf1481` — 훅 `decideNudge` 문구 ①②③(최초·재넛지 동일, 로직 불변 회귀 6/6) hook-cases RED 12→GREEN 12, RL §2i 분기(RL:355)·`## 다음 액션`(RL:375) 바이트 동일, 순서 표·필드 불변, 68,610B(합성값 동일), 리뷰 clean(minor deferred: `(0)`↔② 라벨 교차 참조) |
| 06 | 웨이브 1 GREEN + AC1~3·5·6·8(RL) grep + AC9 | [x] | [task-06](2026-09-21-opshub-field-defect-fixes/task-06-wave1-green-size.md) | 05 | 2026-09-22 커밋 없음(압축·수정 사이클 미발동) — 훅 GREEN 12/12 · AC grep 전항 일치 · AC9 68,610B ≤ 69,066(여유 456B) · COPY_SYNC·RUNS_FRESH · new arm(opus 25런) V 12/12·N0 3/3·N1 5/5(auto 5/5, cur 1/5)·N2 5/5; V1·V4·N0 = 효과 미확인(자동 보완), tdd 기록 GREEN·AC 대조·AC9 절 작성(.remember, 미커밋), 리뷰 clean. **SDD 최종 리뷰 후속(a7e21b2, RL 68,956B)**: Important 3건 수정(AC8a `.git/index.lock` 0건·§2f 선행 커밋 불릿·③ 대기 `timeout` 600000) → 재리뷰 3/3 ADDRESSED → 사본 재생성·RL 의존 20런 재실행 25/25(COPY_SYNC·RUNS_FRESH) |
| 07 | WPS 계약 블록 어댑터·트레일러 — F4·F8(d) | [x] | [task-07](2026-09-21-opshub-field-defect-fixes/task-07-writing-plans-split-contract.md) | 06 | 2026-09-22 `bc5574d` — 40행 블록 순수 삽입(623자: SDD 어댑터 2문장 + 조건부 no-AI-trace, 버전 무표기) + 41행 각주 + §Execution handoff 포인터화, AC4 전항, 리뷰 clean(minor deferred: 각주가 같은 블록인용) |
| 08 | DC·DR·HS·UM 문면 정정 — F7·F8(a) | [x] | [task-08](2026-09-21-opshub-field-defect-fixes/task-08-dev-cycle-doctor-lock.md) | 06 | 2026-09-22 `2b793cb` — DC 28·33·38·83(PR/MR·임계 넛지·merged 보존) · DR 16·26 · HS 151 · UM 104 8행 바이트 동일, `40%` DC/RL 0, AC7·AC8a 전항, 리뷰 clean |
| 09 | README 3종 동기 — F7 + 0.18.0 신규 서술 3건 | [x] | [task-09](2026-09-21-opshub-field-defect-fixes/task-09-readme-sync.md) | 06 | 2026-09-22 `ef222c3` — 3언어 각 6곳(10·83·89·91 삽입·118·190) 동일 위치(SAME_POSITION 91/190), 신규 서술 F1·F5·F6, `0.18.0` ×2, AC7·AC10 전항, 리뷰 clean(minor deferred: JA `。` 뒤 공백) |
| 10 | 릴리스 0.18.0 + 설치 갱신 안내(4머신) + AC11 기록 지침 | [x] | [task-10](2026-09-21-opshub-field-defect-fixes/task-10-release-0-18-0.md) | 07, 08, 09 | 2026-09-22 `6cf5a1b` — plugin.json `0.17.0`→`0.18.0`(JSON ok), 트랙 전체 no-AI-trace 실형 0(4e3cd8d..HEAD 커밋 메시지·변경 파일), README 3종 `0.18.0` ×2(AC10), 말미 `## AC11 실사용 확인` 절(빈 표 2행 + 기록 지침, 브리프 바이트 동일), 리뷰 clean. 미push(사용자 판단). AC11 = 릴리스 후 spark2·Windows 각 1루프 실측이 트랙 완료 조건 |
...
- score 이력(산식 = RL §blocking score, 미확인 FIXED 큐 제외): R1 = 6(high 1 + medium 3, 전부 FIXED 후보 — 그중 1건은 즉시 ESCALATE 경유) · R2 = 2(medium 2, 전부 FIXED 후보) · R3 = 2(medium 2, 전부 FIXED 후보) · R4 = 1(medium 1, FIXED 후보) · R5 = 1(medium 1, FIXED 후보 — R4 수정의 회귀) — 신호 1(2회 연속 비감소) 6→2→2→1→1: R5에서 1 ≥ 1 이나 1 ≥ 2 아님 → 미발화 · 신호 2 미발화 · **신호 3(적대 예산 소진 5/5) 발화 → batch 적재 0건(flush 없음) → 확인 모드 진입**.
- 미확인 FIXED 큐: **0건**(C1에서 fp-OF-P-R1-1 ~ R5-1 10건 전부 소멸 확인 — 수정 커밋 `6698f68`·`63fce1f`·`ec031b4`·`479065e`·`2a58d88`). 적대 소진 5/5 · 확인 소진 1/2 · 복귀 미사용 · 폴백 없음.
- **종료: 성공(확인 경유), 2026-09-22 06:58.** 총 6라운드(적대 5 · 확인 1). 최종 verdict = C1 **pass**. score 추세: R1 6(high 1·medium 3) → R2 2(medium 2) → R3 2(medium 2) → R4 1(medium 1) → R5 1(medium 1) — 전부 신규(잔존·재출현 0), 전환 = 신호 3(적대 예산 소진). disposition 집계: **FIXED 10**(그중 1건 = 즉시 ESCALATE → 사용자 판정 → FIXED) · ACCEPTED 0 · DEFERRED_TO_IMPL 0 · OUT_OF_SCOPE 0 · DUPLICATE 0 · 미해결 ESCALATE 0 · **low 2**(C1 신규 — DEFER_LOW; 수치 오기라 종료 커밋에서 정정: 재실행 런 수 22→20(task-01·06), 합성 중간값 1B 동기(SC-7·task-02~04) — 확인 뒤 변경분은 이 2건뿐). 루프 건강 3지표(분모 = 고유 blocking 10): 재론률 0/10 · 철회 조항 0 · 사람개입률 1/10(fp-OF-P-R1-2 D34 갱신). 단계 경계 인계 항목(ACCEPTED/DEFERRED/OUT_OF_SCOPE) **없음**. impl에 넘기는 사실: D34 = +7KB(69,066B), 하네스 25런(N0 추가), 대기 = `seq 38` 유계 루프, AC11 = 도구 timeout 주입 3단계.
...
| fp-OF-P-R5-1 = task-06 AC(:98) RUNS_FRESH · task-01 PLAN.md · "RUNS_FRESH가 모든 *.md를 RL 사본 mtime과 비교해, RL만 수정하고 N2를 유지하는 §5 재실행 경로에서 N2 5건이 반드시 오래된 결과로 검출 — 행동 검증이 전부 통과해도 필수 AC를 얻을 수 없어 구현자가 범위·AC를 plan 밖에서 재결정" · "RL 사본 비교는 V1~V4·N0·N1로, N1·N2는 훅 사본과 비교, 사본 재생성도 변경된 사본에만, PLAN.md 신선도 설명 동기" | medium | **FIXED** `2a58d88` | task-06 AC RUNS_FRESH를 케이스별 의존성으로 제한(`find … \( V[1-4] -o N[01] \) ! -newer RL 사본` + `N[12] ! -newer 훅 사본`), §5 "바뀐 사본만 재생성", §6·Caution 문구 동기, task-01 PLAN.md 신선도 문장 동기. 루프 세션 시뮬레이션(RL만 갱신·N2 구 파일 유지)에서 통과 확인. R4 수정(fp-OF-P-R4-1)의 회귀 — 같은 fingerprint 아님(범위 정합성 ≠ 재실행 범위). 미확인 FIXED 큐 편입 |
...
- **R5**(적대, 2026-09-22 06:48~06:50, target `46d4da1`, 정밀 모드): verdict needs-attention · 신규 1(medium 1) · FIXED 1 · DUPLICATE 0 · low 0. 유효성: 마커 `COMPANION_EXIT:0` · 헤더 1 · 명령 로그 14건 · `bwrap:` 0 → 유효. 큐 9건 적대 비재출현(R5, 참고 신호 — 큐 유지). 수정 커밋 `2a58d88`. **적대 소진 5 = max → 전환 신호 3 발화 → batch 적재 0건 → 확인 모드 진입(C1).**
- **C1**(확인, `task --prompt-file`, 2026-09-22 06:53~06:58, 스레드 `01a0c5f5-8bc8-7411-b928-c98a0915adf9`, target `72d9bb5`): **완전 응답** — fp-OF-P-R1-1 소멸(task-02:60) · R1-2 소멸(합성 68,610 ≤ 69,066) · R1-3 소멸(task-06:46) · R1-4 소멸(task-02:112, 합성본 0건) · R2-1 소멸(task-02:78 + N0) · R2-2 소멸(task-10:50) · R3-1 소멸(task-02:75) · R3-2 소멸(task-10:37, 합성 RL·WPS 오탐 0) · R4-1 소멸(task-06:75) · R5-1 소멸(task-06:98) · 회귀 (a) SC-4·SC-5 바이트 동일 (b) SC-3 3종 동일 (c) 합성 RL grep 전 요건 충족 (d) 예산·상한 통과 (e) 25런·AC 계수 일치 (f) D34 일관 — 전부 통과 · 판정 감사 해당 없음 · 신규 blocking 없음(low 2) · verdict **pass**. 유효성: 마커 `COMPANION_EXIT:0` · 명령 로그 16건 · `bwrap:` 0.

## 적대검증 ledger (impl)

- 루프: review-loop(impl) 2026-09-22 09:55 시작(spark2 · Fable). base = `5f7adc7`(해소 SHA `5f7adc70e50e8bc2dce29f5abdab52588d1ae152`, plan 종결 커밋 = 구현 착수 직전 main) · branch main · 시작 HEAD `e5869d0`. 예산: max 5 · confirm 2 · auto 3. 게이트(plan §웨이브 경계: npm 없음 → task-06 GREEN 기록 + AC grep으로 갈음): 훅 hook-cases GREEN 12/12 · AC1~AC8 grep 전항 기대값 · AC9 68,956B ≤ 69,066 · 하네스 사본 COPY_SYNC · companion 해소 1.0.6(SC-4 원문, RESOLVE_FAIL 없음). 보안 크리티컬 아님(변경 = 규약 문면 + 훅 문구, 권한·인증·데이터 접촉면 없음). **입도 = 통합 1회**(RL 규약의 task별 증분은 구현 진행 중 직전 task 커밋을 base로 돌리는 형태인데 이 루프는 SDD 종결 후 시작했고 companion은 base..HEAD만 본다 — 전체 diff 11파일 106+/70-라 분할 실익 없음. RL은 task-06 이후라 대상 포함). 실행 방식 = 0.18.0 RL §2b 수동 선적용(설치본 0.17.0) — `.remember/loop-2026-09-21-opshub-field-defect-fixes-impl-R<N>.{sh,out,pid,focus,prompt}` + node `spawn(detached)` + `COMPANION_EXIT:` 마커 + 백그라운드 대기(Bash timeout 600000). R1 래퍼는 수정 전 문면(비인용 heredoc)으로 기동했다 — R2부터 eb38fd8 문면.
- score 이력(산식 = RL §blocking score, 미확인 FIXED 큐 제외): R1 = 3(high 1, FIXED 후보) · R2 = 0(신규 0) — **신호 2(수정 큐 소진: 최신 적대 라운드 신규 FIXED 후보 0건) 발화** → batch 적재 0건(flush 없음) → 확인 필수 조건(미확인 FIXED 큐 1건) 충족·확인 예산 잔여 2 → 확인 모드 진입(C1).
- 미확인 FIXED 큐: **0건**(C1에서 fp-OF-I-R1-1 소멸 확인 — 수정 커밋 `eb38fd8`). 적대 소진 2/5 · 확인 소진 1/2 · 복귀 미사용 · 폴백 없음.
- **종료: 성공(확인 경유), 2026-09-22 10:18.** 총 3라운드(적대 2 · 확인 1). 최종 verdict = C1 **pass**. score 추세: R1 3(high 1, 신규) → R2 0(신규 0·잔존 0) — 전환 = 신호 2(수정 큐 소진). disposition 집계: **FIXED 1** · ACCEPTED 0 · DEFERRED_TO_IMPL 0 · OUT_OF_SCOPE 0 · DUPLICATE 0 · ESCALATE 0 · low 0. 확인 부채: 미확인 FIXED 큐 0건, 폴백 이월 없음. 루프 건강 3지표(분모 = 고유 blocking 1): 재론률 0/1 · 철회 조항 0 · 사람개입률 0/1. 단계 경계 인계 항목(ACCEPTED/DEFERRED/OUT_OF_SCOPE) **없음**. 다음 단계(9 릴리스 task-10)에 넘기는 사실: RL = **68,974B**(여유 92B, 압축 후보 ① 소진 — ②~⑤ 잔여) · RL 문면이 하네스 사본 `review-loop-new.md`와 어긋남(eb38fd8 — 재실행 여부 사용자 판단) · plan SC-5/task-02 §B 래퍼 원문은 0.18.0 RL과 다르다(quoted heredoc + argv — 이 ledger가 이탈 기록, plan 문서 무수정).
- 루프 직접 판정(ACCEPTED/OUT_OF_SCOPE/DEFERRED_TO_IMPL/DUPLICATE): 0건.
- **하네스 주의**: eb38fd8로 RL 문면이 바뀌어 `$H/skills/review-loop-new.md` 사본과 HEAD가 어긋난다(task-06 규칙 = 사본 재생성 + RL 의존 20런 재실행이 원칙). 변경은 §2b ② 래퍼 블록·§확인 실행 1구·§2b 첫 문단 괄호 삭제로 하네스 V·N 케이스가 읽는 축(유효성 판정·넛지)이 아니다 — 재실행 여부는 종료 보고에서 사용자 판단(비용 20 opus 런).

| fingerprint | severity | disposition | 근거 |
|---|---|---|---|
| fp-OF-I-R1-1 = RL §2b ②(:283-286) · "래퍼에 경로를 보간하면 파일명 속 명령이 실행됨 — 비인용 heredoc이 `$L`·`$PWD`·`$ROOT` 값을 셸 소스에 삽입하고 bash가 재해석, ledger basename에 `$(...)`가 있으면 루프 세션 권한으로 실행(재현됨), `$` 포함 경로도 변형" · "quoted heredoc으로 생성하고 cwd·companion 경로·L·base를 spawn argv로 넘겨 래퍼 안에서 인용된 위치 인자로 사용" | high | **FIXED** `eb38fd8` | 루프 재현: basename에 `$(printf INJECTED >&2)` 삽입 시 heredoc 단계에서 실행 확인. 수정: `<<'EOF'` + `cd "$1"` · `"$2/scripts/…"` · `"$3.focus"` · `--base "$4"`, node 1줄은 `[sh,out,pid,...a]`·`spawn("bash",[sh,...a])`로 `"$PWD" "$ROOT" "$L" <base SHA>`를 argv로 전달(D4 스크립트 비동봉·node 1줄 유지), ② 산문 1구 추가, §확인 실행의 래퍼 명령을 `"$2/…" task --prompt-file "$3.prompt"`로 동기. 검증: 같은 주입 basename + `$HOME` 포함 ROOT로 가짜 companion 기동 → 주입 미실행·argv 원형(`--base deadbeef -- FOCUSTEXT`)·마커 0. AC9: 압축 후보 ①(§2b 첫 문단 실측 괄호 삭제) 적용 → 68,974B(여유 92B). 게이트 재실행 GREEN·AC grep 전항 불변. plan SC-5·task-02 §B 원문은 손대지 않는다(plan은 종결 문서 — 이 행이 이탈 기록). 미확인 FIXED 큐 편입 |
