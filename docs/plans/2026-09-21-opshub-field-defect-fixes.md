...
- score 이력(산식 = RL §blocking score, 미확인 FIXED 큐 제외): R1 = 6(high 1 + medium 3, 전부 FIXED 후보 — 그중 1건은 즉시 ESCALATE 경유) · R2 = 2(medium 2, 전부 FIXED 후보) · R3 = 2(medium 2, 전부 FIXED 후보) — 신호 1(2회 연속 비감소) 미발화(6→2→2), 신호 2 미발화(신규 FIXED 후보 2).
- 미확인 FIXED 큐: **8건**(fp-OF-P-R1-1 ~ R1-4 수정 커밋 `6698f68` · fp-OF-P-R2-1 ~ R2-2 수정 커밋 `63fce1f` · fp-OF-P-R3-1 ~ R3-2 수정 커밋 `ec031b4`). 적대 소진 3/5 · 확인 소진 0/2 · 복귀 미사용 · **적대 소진 == auto-rounds(3) → batch flush 지점: 적재 0건이라 flush 없음 → 정밀 모드 진입**(R4부터 ESCALATE 즉시 처리).
