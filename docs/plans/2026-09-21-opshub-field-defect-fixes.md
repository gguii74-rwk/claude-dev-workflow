...
- score 이력(산식 = RL §blocking score, 미확인 FIXED 큐 제외): R1 = 6(high 1 + medium 3, 전부 FIXED 후보 — 그중 1건은 즉시 ESCALATE 경유) · R2 = 2(medium 2, 전부 FIXED 후보).
- 미확인 FIXED 큐: **6건**(fp-OF-P-R1-1 ~ R1-4 수정 커밋 `6698f68` · fp-OF-P-R2-1 ~ R2-2 수정 커밋 `63fce1f`). 적대 소진 2/5 · 확인 소진 0/2 · 복귀 미사용 · 자동 모드(batch 적재 0건).
