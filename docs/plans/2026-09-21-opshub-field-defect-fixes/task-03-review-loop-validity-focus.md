...
```markdown
**④ 완료·유효 판정 — 적대·확인 공통 순서.** ⓐ 마커 `COMPANION_EXIT:` 존재 = 완료. 마커 없이 프로세스가 사라졌으면(③) **실행 실패**. ⓑ 적대: `sed -n '/^# Codex Adversarial Review/,$p' "$L.out"`로 본문을 추출해 **헤더 + 스키마 JSON**(`{ verdict, summary, findings[{severity,title,body,file,line_start,line_end,confidence,recommendation}], next_steps }`) + **`[codex] Running command:` 실행 로그 ≥1건**을 확인한다 — 정적 검토도 셸 명령(cat·sed·git)이라 로그를 남긴다. 0건 = 아무것도 읽지 않았다 = 실행 실패(finding 0이 아니다). ⓒ **샌드박스 기동 실패 표지**(`bwrap:` · 임시 디렉터리 생성 실패 · 비-JSON 종결 메시지 — 열린 예시)가 실행 로그 **전반**에 걸치면 실행 실패 — "No material findings"·approve JSON이 붙어 있어도 그렇다. **개별 명령 실패는 무효 사유가 아니다** — 고정 첫 줄(§기결정 가드)이 정적 검토 완료를 지시하므로 시도 실패 뒤 완주한 리뷰는 유효(90초 만의 approve도 헤더·로그가 있으면 유효). 확인 응답은 §응답 완전성 계약. **실행 실패·스키마 불일치 처리** = 적대 소진·확인 소진 미반영 · 원문 보고 · 자동 재실행 금지(재실행은 사용자 판단, 추측 금지). companion 미설치/미인증 실패는 멈추고 `/codex:setup`(임의 수정 금지).
```
...
교체:
```markdown
- **빈 가드 = 가드 블록 미포함 — focus 인자는 고정 줄로 항상 부착**: 재논의 금지 블록·닫힌 ledger 항목·미확인 FIXED 큐가 **모두 없으면** `$L.focus` = 고정 첫 줄만(가드 블록·빈 목록을 보내 "닫힌 게 없다"는 신호로 오해될 여지를 만들지 않는다 — guard-focus D10의 취지, 문언만 정밀화). **미확인 FIXED 큐만 비어 있지 않으면 고정 줄 + 진행 상태 한 줄**(§기결정 가드 — 계열 B 고지가 빈 가드 분기로 소실되지 않게).
```
...
```markdown
  - **고정 첫 줄 — 적대 focus 맨 앞, 매 라운드, 무효 선언형 짝 바깥**: "샌드박스는 읽기 전용이라 테스트·빌드 실행이 실패할 수 있다 — 실패를 이유로 검토를 중단하지 말고 정적 검토로 완료하라(게이트는 루프가 실행한다). typecheck 같은 읽기 전용 명령은 그대로 실행하라." 실사례: 샌드박스 명령 실패 뒤 빈 응답 3건(08-29~09-16), 첫 줄 지시 후 재발 0. **적대만** — 확인 `task` 프롬프트에는 넣지 않는다(확인 실사고 0). 가드 목록이 아니므로 "빈 가드" 판정에 세지 않는다(§2b ②).
```
