...
```markdown
**진행 중 라운드 분기(경로 ①에서 라운드가 백그라운드로 도는 중이면)** — 새 단위를 시작하지 않는다. **작업 단위 = 리뷰 라운드 1회 · SDD task 1개 · 수정 1건 · 질문 라운드 1회**(phase·루프 전체가 아니다 — 훅 문구와 같은 예시). 순서: 응답 수신까지 대기(백그라운드 대기 그대로 — 완료 알림으로 깨어난다. 완료 전 /clear 금지 = §2b 진행 중 금지 (1)) → **순서 0**(§2j 카운터 반영 — **응답 수신 사실만, §2j 표 그대로**: 일반 예산 라운드만 적대 소진 +1 / 확인 소진 +1(완전 응답일 때만 — 완전성은 첨부 큐 fingerprint 대조라 판정 없이 기계적으로 확인된다. 부분 응답이면 미반영·현행 실행 실패 처리), **예약분(복귀 적대·재진입 확인·폴백②)은 어느 카운터도 올리지 않는다**) → 결과를 `## 미해결 ledger`에 **미판정**으로 기록 → 순서 1~4. 판정·수정·다음 라운드(auto-rounds 경계의 batch flush 포함)는 재개 세션 몫이다. **재개 계약**: 미판정 응답의 판정 정책 = **수신 시점 모드**(카운터 증가 **전** 소진으로 정해진 자동/정밀·적대/확인). 재개 세션은 `## 다음 액션`에 기록된 그 모드로 §2c·§2d를 처리하고 **카운터를 다시 올리지 않는다**. 결과 의존 전이(score 스냅샷·전환 신호·auto-rounds 경계 batch flush·확인 결과 처리의 복귀/종료)는 판정 뒤 확정한다 — 예: auto-rounds=3에서 R3 응답 수신 후 중단하면 소진=3이 저장되지만 R3 판정은 **자동 모드(batch 적재)**로 하고 그 뒤 flush → 정밀 모드(중단 없는 경로와 같은 정책). 예약분 라운드 중단도 두 카운터 불변·후속 전이는 중단 없는 경로와 같다. **새 필드는 없다** — 규약 위반(/clear 강행)·대기 사망 뒤 완료된 고아 라운드는 재실행한다(응답 미수신 = 예산 미소모). 출력 파일은 규칙 경로(§2b ②)라 재개 세션이 볼 수 있다.
```
