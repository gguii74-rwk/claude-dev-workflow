...
**목적**(릴리스·push·맥북 설치 갱신 뒤 — SDD 범위 밖): **릴리스된 최신 1.0.x 설치본**(최소 1.0.0 — 복구 절차 뒤에는 그 patch)으로 맥북 오르카 터미널에서 실제 트랙 세션 1회를 관찰해 엔트리포인트 `## AC6 실사용 확인 (트랙 완료 조건, D11)` 표 11행의 관찰·결과 열을 채우고, eval 보고서에 부기한다. task-05가 만든 **빈 표·절차**가 입력이다. **이 task의 완료 커밋 = 트랙 완료**(dev-cycle 9단계 완료 신호, D11). task-05는 릴리스 커밋으로 완료되고 AC6 증거는 여기서만 닫힌다(plan R3-3).
...
- 사전 조건 확인: 1.0.0 push 완료 · 맥북에서 `/plugin update dev-workflow@claude-dev-workflow` → 재시작 → `/dev-workflow:doctor`가 설치본 버전(1.0.0 또는 그 뒤 patch)을 보고 — **그 값을 AC6 절 상단 `**설치 버전**:` 줄에 적는다**(표·eval·커밋 문구의 버전도 이 값) · `orca terminal list --worktree active --json` 동작(맥북 오르카 클라이언트 접속 중).
...
- 어느 행이든 `실패`면 **복구 절차**(plan R5-3): 원인 수정 → review-loop(impl) 재진입(impl ledger에 라운드 추가) → patch 버전 bump(`release: 1.0.1 — …`, 이후 1.0.2 …)·push(사용자) → 맥북 `/plugin update` → 재시작 → 단계 1부터 AC6 재실행. 표는 마지막 회차로 덮어쓰고 이전 회차는 비고에 요약, `**설치 버전**:`은 재실행한 patch로 갱신.
