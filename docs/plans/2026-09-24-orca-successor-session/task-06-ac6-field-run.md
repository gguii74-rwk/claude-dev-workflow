...
task-05(릴리스 커밋) + push(사용자 판단) + 맥북 설치 갱신. **SDD 디스패치 대상 아님** — 9단계 이후 이 파일을 단독 실행한다(엔트리포인트 "SDD 실행 범위 = task-01~04").
...
- 그 세션에서 **실제 트랙**의 review-loop를 시작한다 — §0~§2b가 한 턴 안에서 라운드 기동까지 가고 백그라운드 대기로 턴이 끝나는 Stop이 첫 넛지가 되도록(조건 ③ — 11행). 작업명은 메타문자를 섞어 짓는다(조건 ② — 2·10행, 기대값은 task-05 절 원문).

### 2. 관찰·기록

- 넛지 → 핸드오프 → 후계 스폰 → 옛 세션 종료(SessionEnd 정리) → 후계 §0 재개까지 지켜보며 표 11행의 **관찰 열**(명령 출력 요지·시각)과 **결과 열**(`통과` / `실패`)을 채운다. 결과 열은 이 두 값만 쓴다(AC가 문자열로 센다).
...
awk '/^## AC6 실사용 확인/{f=1;next} /^## /{f=0} f' $P | grep -E '^\| [0-9]+ \|' | awk -F'|' '{gsub(/^ +| +$/,"",$5); gsub(/^ +| +$/,"",$6); if ($5 != "" && $6 == "통과") n++} END {print n+0}'   # 11 — 관찰 열 비어 있지 않고 결과 열 = 통과, 11행 전부(빈 표·부분 기록·실패 행이 있으면 < 11)
