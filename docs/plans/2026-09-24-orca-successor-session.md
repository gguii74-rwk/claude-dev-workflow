...
- **경로 = 정식**(사용자 확정 2026-09-24). 3.5 비대상. 다음 = 6 review-loop(plan) → 7 impl(SDD, **task-01~04만**) → 8 review-loop(impl) → 9 릴리스 1.0.0(**task-05 — SDD 범위 밖, 8 종결 뒤 단독 실행**) → **AC6 실사용 1회 = task-06**(1회차 맥북 · 2회차 spark2 — 사용자 판정 A)(트랙 완료 조건, D11 — 표가 채워진 커밋이 완료 신호).
...
- [폴백]: `[폴백] 조건 4종: (a)` · `(b) (2-2) wait satisfied:false` · `(c) (2-3) turn_started 미확인` · `(d) CLI 실행 오류` · `생성 전후 핸들 집합 차분으로 후보만 산출` · `create 응답 유실은 예외 — 새 핸들을 모르므로 아래 정리 없이` · `재생성과 /clear 안내를 모두 차단` · `미전달이 확정되기 전(재관찰·재조정 중)에는 후계를 닫지 마세요` · `"$CLI" terminal wait --terminal "<새 핸들>" --for exit --timeout-ms 30000 --json` · `/exit 처리 증거 없이 close하지 마세요` · `소멸을 확인하고` · `소멸이 확인된 경우에만` · `/clear를 안내하지 말고 차단 상태` · CLEAR (F2·R1-3·D10·plan R4-1 — 정리 미확인이면 안내 없이 보고·정지)
...
- 부정 요건: `자가 /clear는 불가` 0건 · `CLAUDE_CODE_DISABLE_TERMINAL_TITLE`·`title이 정확히`·`title 정확 조회`·`정확히 1개가 아니면`·`회수를 먼저 시도` 0건(1.0.2) · `"$CLI" terminal (send|wait|close|read|show)` 뒤에 ` --terminal ` 없는 것 0건 · 옛 핸들 바인딩은 값 리터럴(`--terminal "term_…"`), 새 핸들은 `"<새 핸들>"` 자리표시자만 · 안전 문자 집합 밖 `ORCA_TERMINAL_HANDLE`은 오르카 경로에 들어가지 않는다(`resolveOrcaHandle` → null, E2E C8).
...
### SC-7. AC6 실사용 확인 절 (task-05가 빈 표·절차를 말미에 추가, task-06이 릴리스 후 spark2 세션에서 채운다 — D11, 1회차 맥북)

절 제목 `## AC6 실사용 확인 (트랙 완료 조건, D11)`. 관찰 항목 표 11행(사전 검증 · 제목 정규화 · wait · turn_started · 옛 세션 정지 · 후계 0번째 · 첫 동작 옛 핸들만 · SessionEnd 정리 · §0 대조·라운드 이어감 · 메타문자 원문 전달 · 라운드 진행 중 넛지). **11행 전부 필수**(11행 "미발생" 불허 — spec F6 파일럿 미측정 3건은 9·11행이 닫는다, plan R1-3). 메타문자 전달 프로브 = 재개 프롬프트 템플릿에 내장된 `$(…)` 식(CLI·companion 해소식)이 확장 없이 도착하는지(`<경로>` 슬롯은 review-loop에서 고정 — plan R1-4). 채워진 뒤 eval repo `report/ORCA-SUCCESSOR-2026-09-24.md`에 부기(파일럿 미측정 3건 종결) — task-06. 배포 순서 = push → AC6 머신만 갱신(2회차 = spark2) → task-06 통과 → 나머지 머신(plan R4-2). AC6 대상 버전 = 릴리스된 최신 1.0.x(최소 1.0.0; 실패 복구 후엔 그 patch) — 실제 설치 버전을 절 상단 `**설치 버전**:` 줄에 적고 task-06 AC가 검사한다(plan R5-3). 결과 열은 `통과`/`실패`만: task-06 AC가 11행 전부 관찰 열 비어 있지 않고 `통과`인지 센다(빈 표·부분 기록으로는 06행이 `[x]`가 되지 않는다, plan R3-3).
...
| 06 | AC6 실사용 확인 기록 — **트랙 완료 조건** (F6·D11) — **릴리스·push·spark2 갱신 뒤, SDD 디스패치 대상 아님** | [x] | [task-06](2026-09-24-orca-successor-session/task-06-ac6-field-run.md) | 05 + push + spark2 설치 갱신 | 3회차(spark2 1.0.2) 11/11 통과 — 1회차 맥북 1.0.0 2행 실패 → 1.0.1 · 2회차 spark2 1.0.1 2·3행 실패 → 1.0.2. **eval 부기 잔여** = 보고서가 맥북 로컬에만 있어(미push) 맥북에서 부기·커밋 |
...
| 계약 · [F2-2] plan SC-5·task-01 테스트 원문·task-02 구현 원문(재생성 입력)이 제거된 `--command claude --json`을 계속 요구 — 저장소만으로 테스트를 재구성하면 올바른 수정본이 실패·결함 재도입 위험 · 확정 명령과 `--json → 새 핸들` 경계로 동기화 | medium | FIXED `65b8a6b` (자동 모드) | SC-5(66행)·task-01(91행)·task-02(145행)·spec F2 계약(44행) 동기화 — spec 10행은 파일럿 실측 사실이라 불변. task-01 원문에서 재생성한 테스트 = 1.0.1 훅 GREEN 11/11 · `3fbc355` 훅 C4 RED(pass 10 / fail 1). 기존 드리프트(impl 라운드 needle은 외부 정본에만 — 이번 변경 이전부터)는 task-01 원문 앞 정본 안내 1줄로 명시(D5 — 테스트 repo 파일 없음 유지) |
| 릴리스 · [F2-1] manifest가 1.0.0 그대로라 수정본이 사용자에게 배포되지 않음 · 확인 루프 뒤 1.0.1 릴리스 커밋 | high | ACCEPTED **[루프 판정]** — 보완 단계 = 이 루프 종결 뒤 task-06 복구 절차의 `release: 1.0.1` 커밋(순서 계약: 원인 수정 → review-loop(impl) → patch bump) | 결함 아님 — 복구 절차가 이미 정한 다음 단계를 앞당겨 요구한 것. 루프 중 bump하면 리뷰 범위가 릴리스 커밋까지 섞인다. **확인 임무 ③ 우선 감사 대상** |
...
|---|---|---|---|---|
| 1 | (2-0) 사전 검증·게이트 | `terminal show` ok:true · `GATE_OFF FOREIGN_ACTIVE=0` | 20:10:31 옛 세션 `term_53eb78f7…`: `terminal show` ok:true(orphaned:false) · 상태 프로브 `GATE_OFF FOREIGN_ACTIVE=0` · (2-0)ⓓ 생성 전 목록 ok:true·`truncated:false`·1건(자기 핸들) | 통과 |
| 2 | (2-1) 제목 정규화·토큰 | 제목 `[A-Za-z0-9._-]`만, ≤40, 끝 토큰 온전 · (2-0)ⓓ 생성 전 목록 ok·`truncated:false` · 생성 뒤 `terminal list`에서 생성 전 목록에 없던 새 핸들 정확 1건 = create 응답 핸들(1.0.2 — 제목 정확 조회 폐기) | TOKEN `201039886t` · 제목 `ac6---echo-x---id--201039886t`(29자, 영숫자·하이픈만, 토큰 온전) · 생성 뒤 목록 2건 중 생성 전 목록에 없던 새 핸들 = `term_c12ac152…` 1건 = create 응답 핸들(main 관찰 list 20:10:42). 탭 제목은 claude가 `Claude Code`로 덮어씀(1.0.2 — 제목 조회 미사용이라 무관) | 통과 |
| 3 | (2-2) 기동 대기 | `result.wait.satisfied: true` (90s 이내) | 20:10:44 `{"condition":"tui-idle","satisfied":true}` — create(20:10:3x) 뒤 약 10초 | 통과 |
| 4 | (2-3) 전달 | `result.send.prompt.stages`에 `turn_started` · `--retry-request` 사용 여부 | 20:11:13 파일 전달(`.remember/successor-201039886t.prompt`) → accepted:true · stages `input_accepted`·`turn_started` · `--retry-request` 미사용(1회 전송) · 전달 뒤 프롬프트 파일 삭제 | 통과 |
| 5 | (2-5) 옛 세션 정지 | send 뒤 옛 세션 추가 턴 0(재넛지 없음 — `stop_hook_active` 통과) | send 뒤 옛 세션은 같은 턴의 인계 보고 1건(20:11:25)으로 종료 · stop_hook_summary 차단 없음(재넛지 0) · 추가 턴 0 | 통과 |
| 6 | 후계 0번째 동작 | `FOREIGN_ACTIVE=0` 확인 후 진행(대기 발생 시 횟수) | 20:11:23 후계 `b5cf56c5`: `GATE_OFF FOREIGN_ACTIVE=0`(CODEX_COMPANION_SESSION_ID 자기 세션) · 대기 0회 | 통과 |
| 7 | 후계 첫 동작 — 옛 핸들만 | `/exit` accepted → `--for exit` satisfied, 또는 시간 초과(보통) 뒤 `terminal read` 끝줄에 종료 표지 `Resume this session with`와 셸 프롬프트 둘 다 — **표지 출력 여부를 관찰 열에 기록**(빈 세션은 표지 없음이 실측, 대화가 있는 세션은 미관찰 — impl 이월 F6) → close ok → list에 옛 핸들 없음 · **활성 탭·다른 탭 무사** | 20:11:29 `/exit` accepted:true → `--for exit` 30s 시간 초과(`error.code timeout`, 보통) → read 끝줄에 종료 표지 `Resume this session with`(출력됨 — 대화 있는 세션) + 셸 프롬프트 둘 다 → 20:12:06 close ok(ptyKilled:true) → list grep 0 · 활성 탭(main 세션, 다른 워크트리) 무사 · 후계 자신 무사 | 통과 |
| 8 | SessionEnd 정리 | 옛 세션 codex 상태 디렉터리: `broker.json` 소멸 · 옛 세션 잡 0(파일럿 E5와 동일) | 20:11:34 옛 세션 SessionEnd — codex 상태 디렉터리 `broker.json` 소멸 · state.json jobs 0(R2 completed 기록 정리) | 통과 |
| 9 | §0 대조·라운드 이어감 | 후계가 `/review-loop --resume`(또는 핸드오프)로 §0 스냅샷 통과 · 다음 단위 시작 · 단계 경계면 사용자 확인(D4). 관찰 열에 대상 트랙의 repo · phase · ledger 문서 · base SHA · 루프 파일 경로 기록(이 트랙 ledger 재사용 아님) | 후계 `/dev-workflow:review-loop --resume` → §0 스냅샷 전부 일치(브랜치·HEAD `d806434e8`·base `57cb21fe2`·clean) → R2(finding 0) 판정 → 확인 모드 C1 진입(20:12:57 codex running). repo = ops-hub(`feat/meetings-speaker-quickpick` 워크트리) · phase = impl · ledger = `docs/plans/2026-09-24-meetings-speaker-quickpick.md` · base `57cb21fe2` · 루프 파일 `.remember/loop-2026-09-24-meetings-speaker-quickpick-impl.md`. 단계 경계 아님 | 통과 |
| 10 | 메타문자 원문 전달 | 후계가 받은 프롬프트에 CLI 해소식 `$( [ -n "$ORCA_DEV_REPO_ROOT" ] && echo orca-dev … )`(원문은 조건 ② — 셀 안에 세로줄을 쓰지 않는다)·`$(node -e` 원문 그대로 · `CLI="orca"` 등으로 확장된 흔적 없음 · 옛 세션 로컬 실행 흔적 없음 | 후계 첫 사용자 메시지(transcript `b5cf56c5`, 4,321자): `ORCA_DEV_REPO_ROOT` 원문 2회(해소식 포함) · `$(node -e` 원문 1회 · `CLI="orca"` 0회 · 옛 세션은 heredoc(`'SUCCESSOR_PROMPT_EOF'`)으로 파일 전달해 로컬 확장 없음 | 통과 |
| 11 | codex 라운드/서브에이전트 진행 중 넛지 (**필수**) | 라운드 진행 중 Stop에서 넛지 → 완료 알림까지 새 단위 없음 → 응답 결과를 루프 파일 `## 미해결 ledger`에 미판정으로, `## 다음 액션`에 수신 라운드 번호·판정 정책(수신 시점 모드)·미처리 단계를 기록 → 스폰(§2i 진행 중 라운드 분기) | R2 기동 20:08:43(codex running) → 같은 턴 Stop(20:08:46)에서 넛지(사용량 약 16%) → 「R2가 백그라운드에서 돌고 있어 끝날 때까지 기다림」 → 20:09:45 R2 completed(approve·finding 0) → 새 단위 없이 루프 파일 기록(적대 소진 2·R2 판정 정책 = 수신 시점 모드 = 자동·미판정) → 20:10:3x 스폰 | 통과 |
...
- **비고 — 2회차(2026-09-24 19:22~19:35, 1.0.1 설치본, spark2 Orca 1.4.209 · bash) = 실패 2행(2·3행) → 폴백 (b) → 복구 절차**: 대상 = ops-hub `feat/meetings-speaker-quickpick` review-loop(impl), ledger `docs/plans/2026-09-24-meetings-speaker-quickpick.md`, 구현 커밋 `598e7945c`(origin/main `57cb21fe2` 분기), 루프 파일 `.remember/loop-2026-09-24-meetings-speaker-quickpick-impl.md`(ops-hub 워크트리 기준), 옛 세션 `term_fb32aa75…`(`CLAUDE_CTX_THRESHOLD=0.05`), 활성 탭 = 다른 탭(main 세션) 유지. 타임라인: 19:22:07 첫 메시지 → 보안 트랙 질문(일반 선택) → 19:27:47 codex R1 running · 19:27:53 `broker.json` 생성 → 19:28 R1 진행 중 Stop에서 넛지 → 옛 세션 「R1이 백그라운드에서 돌고 있어 끝나기 전엔 인계 안 함」 → 19:29:15 R1 completed → 응답 유효 판정·소진 1·medium 1 미판정 기록 → 인계 시작 → 19:30:17 후계 `term_ce31af4a…` 생성(옛 세션 보고 제목 `ac6---echo-x---id--193012132a` — 정규화는 기대와 일치) → `terminal wait --for tui-idle` 90s + 재시도 180s 모두 미충족 → **폴백 (b)** → 브로커 잡 0 확인 → 후계 `/exit`·close·list 소멸 확인 → 「/clear 후 `--resume`」 안내 → 19:35 종료 · 19:35:11 `broker.json` 소멸(후계 claude `/exit`의 SessionEnd — 폴더 공유 브로커). 재개 프롬프트 미전송 → 4행 이후 미도달. 행별: 1 = 옛 세션이 `terminal show`·게이트 프로브 실행 뒤 create로 진행(폴백 (a)가 아니었으므로 ok:true·GATE_OFF로 추정 — 출력은 화면 접힘으로 main 캡처에 없음, 통과로 단정하지 않음) · **2 실패**(원인 B) · **3 실패**(원인 A) · 4~10 미도달 · 11 = 넛지 → 완료 대기 → 미판정 기록 → 스폰 시도까지 기대대로(스폰이 폴백으로 끝남). **원인 A — 1.0.1 복구가 (2-2) 기동 대기를 깨뜨린 회귀**: 오르카 `terminal wait --for tui-idle`은 claude가 쓰는 탭 제목(`✳ Claude Code`)으로 idle을 판정하는데 `CLAUDE_CODE_DISABLE_TERMINAL_TITLE=1`(`ec0d728`)이 그 제목 쓰기를 막아 idle 신호가 오지 않는다. 대조 프로브(19:36, ops-hub main 워크트리, 기동 직후 wait 45s): 환경변수 있음 → `ok:false` `timeout`(45s) · `terminal show` title = 셸 제목 / `claude` 그대로 → `satisfied:true` 1s · title = `✳ Claude Code`. 1회차(맥북 1.0.0)는 이 변수가 없어 wait 3s 통과, 1.0.1 복구 뒤 실측은 이 2회차가 처음. **원인 B — spark2 bash가 `--title`을 즉시 덮어씀**: Ubuntu 기본 `~/.bashrc` `PS1="\[\e]0;…\u@\h: \w\a\]$PS1"`(xterm*)가 첫 프롬프트에서 OSC 0으로 제목을 `uracle-spark@spark-02: ~/…`로 바꿔 claude가 뜨기 전에 이미 `terminal list` title 정확 조회 0건 — 「create 응답 유실 시 title 정확 조회로 회수」 경로는 claude 설정과 무관하게 셸에 따라 성립하지 않는다(맥북 zsh는 1회차에서 claude 기동 전까지 제목 유지 관찰). 이번 인계는 create 응답의 핸들을 써서 B의 영향은 없었지만 2행 기대(「title 정확 1건」)는 이 머신에서 충족 불가. 폴백 정리는 기대대로(반쪽 터미널 소멸 확인 뒤 /clear 안내).

- **비고 — 3회차(2026-09-24 19:59~20:12, 1.0.2 설치본, spark2 Orca 1.4.209 · bash) = 표 기록 회차, 11행 통과**: 대상 = 2회차와 같은 ops-hub `feat/meetings-speaker-quickpick` review-loop(impl)의 남은 라운드(R1 판정·수정 `6c61c2573` → R2). 설치 = push `74c0f4d..4ebc8bd` → spark2 user·ops-hub project·트랙 워크트리 project 스코프 1.0.2(`4ebc8bd`, subtree `267eeac` = origin/main) — 트랙 워크트리 project 엔트리는 `plugin update --scope project`가 「already latest」로 1.0.1에 남아 uninstall → install로 갱신(`.claude/settings.json` 변경은 원복). 옛 세션 `deb4d346`(`term_53eb78f7…`, `CLAUDE_CTX_THRESHOLD=0.05`) → 후계 `b5cf56c5`(`term_c12ac152…`). 활성 탭 = 다른 워크트리의 관찰(main) 세션. 부수: 넛지 턴 화면의 `Stop hook error occurred`는 넛지 전달 방식(hook_blocking_error) 표시다.
...
- 채우는 절차·주체 = **task-06**(spark2에서 그 세션을 관찰한 사람/후계 세션 — 이 파일 커밋). task-06의 통과 커밋이 **트랙 완료**(dev-cycle 9단계 완료 신호). 결과 열은 `통과`/`실패`만(task-06 AC가 센다).
- Windows는 트랙 밖(D11 후속) — 각 머신 첫 넛지 때 1·3·4행(`ORCA_TERMINAL_HANDLE` 존재 · `orca`/`ORCA_CLI_COMMAND` 해소 · `--wait-submit` 지원)만 확인하고 실패 시 폴백으로 현행 동작임을 기록한다.

**설치 갱신(4머신) — 순서가 게이트다(plan R4-2)**: ① push(사용자 판단) → ② **spark2만** (2회차 머신 = spark2 — 사용자 판정 A 2026-09-24: 실제 작업이 spark2에 몰려 있어 "맥북 먼저" 게이트를 "spark2 먼저"로 바꿨다. 1회차는 맥북 1.0.0) `/plugin update dev-workflow@claude-dev-workflow` → 재시작 → `/dev-workflow:doctor`(설치본 1.0.0) → ③ task-06(AC6 실사용 확인) 통과 → ④ 나머지(OMEN `D:\workspace` · 그램 `C:\workspace`) 같은 절차로 갱신(맥북은 1회차 때 갱신). AC6가 실패하면 task-06 복구 절차(원인 수정 → review-loop(impl) 재진입 → patch bump·push → spark2 갱신 → AC6 재실행; 증거 버전 = 그 patch) → 그 뒤 나머지 머신. project 스코프로 고정된 repo(ops-hub 등)는 그 repo 안에서 `claude plugin update dev-workflow@claude-dev-workflow --scope project`.
