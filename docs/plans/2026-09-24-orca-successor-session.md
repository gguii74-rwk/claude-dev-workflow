...
- **경로 = 정식**(사용자 확정 2026-09-24). 3.5 비대상. 다음 = 6 review-loop(plan) → 7 impl(SDD) → 8 review-loop(impl) → 9 릴리스 1.0.0 → **AC6 맥북 실사용 1회**(트랙 완료 조건, D11).
...
| 05 | 릴리스 1.0.0 + 4머신 갱신 안내 + AC6 절 (F4·F6) | [ ] | [task-05](2026-09-24-orca-successor-session/task-05-release-1-0-0.md) | 04 | |

**AC ↔ task**: AC1·AC2 → 02(01 테스트 C1~C6·C8) · AC3 → 03 · AC4 → 04(README)·05(plugin.json·4머신) · AC5 → 02(기록 절) · AC6 → 05(릴리스 후 실사용 — 트랙 완료 조건).

**review-loop(impl) 입도**: 통합 1회(task 5개, 코드 1파일). base = 구현 착수 직전 main SHA(plan 종결 커밋). **8단계 impl 게이트**: 이 repo는 npm이 아니므로 RL 게이트 4종 대신 **SC-6 GREEN 기록 + 각 task AC grep**으로 갈음한다(dev-cycle 규약 · spec §6).
