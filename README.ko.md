...
1.0.0부터 **오르카 터미널**(`ORCA_TERMINAL_HANDLE`이 있는 세션)에서는 넛지의 `/clear` 안내가 **자동 인계**로 바뀐다: 핸드오프를 쓴 뒤 같은 체크아웃에 후계 `claude` 터미널을 직접 띄우고(제목 `<작업명>-<토큰>`), 재개 프롬프트를 파일(`.remember/successor-<토큰>.prompt`)로 조립해 보낸 다음 정지한다. 후계는 이 폴더에서 다른 세션의 codex 작업이 돌고 있지 않은지 확인한 뒤(최대 10분 대기) 첫 동작으로 옛 세션을 정상 종료시키며(`/exit` 전송 → 종료 확인 → `terminal close`), 그 전에는 codex를 실행하지 않는다. 사전 검증에서 실패하면 아직 만든 터미널이 없으므로 바로 현행 `/clear` 안내로 돌아가고, 생성·기동·전달 어느 단계가 실패하면 만들다 만 터미널을 먼저 정리하고 소멸을 확인한 뒤에만 현행 `/clear` 안내로 돌아간다 — 후계를 하나로 확정할 수 없거나(생성 응답 유실, 제목 일치 0 또는 2+) 정리를 확인하지 못하면 `/clear`를 안내하지 않고 차단 상태를 보고한다. 단계 경계(spec→plan, plan→impl)의 `/clear`는 대상이 아니다 — 후계는 다른 단계를 시작하지 않고 사용자에게 확인한다. 오르카 밖에서는 이전과 같다.
