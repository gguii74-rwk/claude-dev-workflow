...
0.19.0 からは、ループがユーザーに尋ねる質問を**読み手が finding・ledger の原文を見ていない前提**で書く — 項目ごとに平易な説明 + 具体的な場面の例 1 つ、最初の選択肢 = 推奨（「(추천)」表示）、選択肢名は平易な行動名の後ろに括弧で判定コード（例:「지금 고친다 (FIXED)」=「今直す」）、batch 提示の最後には進行の質問を 1 つ付ける（すべて維持して続行 / 選んで巻き戻す / 今止めて自分で見る）。
...
0.19.0 からは、質問本文を**ギャップ台帳・spec 原文を見ていない人**が読む前提で書く — 平易な説明 + 具体的な場面の例 1 つ、ギャップ ID・D 番号は末尾の括弧に参照としてのみ置く。選択肢名は平易な行動名にし、説明にはその選択肢を選ぶと何が変わるかを書く。
...
1.0.0 からは **Orca ターミナル**（`ORCA_TERMINAL_HANDLE` があるセッション）では、ナッジの `/clear` 案内が**自動引き継ぎ**に変わる: ハンドオフを書いた後、同じチェックアウトに後継の `claude` ターミナルを直接起動し（タイトル `<作業名>-<トークン>`）、再開プロンプトをファイル（`.remember/successor-<トークン>.prompt`）として組み立てて送信し、停止する。後継は、このフォルダで他のセッションの codex ジョブが動いていないことを確認した後（最大 10 分待機）、最初の動作として旧セッションを正常終了させ（`/exit` 送信 → 終了確認 → `terminal close`）、それまで codex を実行しない。事前検証で失敗した場合はまだターミナルを作っていないので、すぐに従来の `/clear` 案内に戻る。作成・起動・送信のいずれかが失敗すれば、このフォルダで他のセッションの codex ジョブが終わるのを待ってから（最大 10 分）作りかけのターミナルを片付け、消滅を確認した後にのみ従来の `/clear` 案内に戻る — 後継を一つに確定できない（作成応答の喪失 — 作成直前の一覧にない新しいハンドルは候補として報告するだけで、1 つでも採用しない）か、ジョブ確認が通らないか、片付けを確認できなければ、`/clear` を案内せず遮断状態を報告する。段階境界（spec→plan、plan→impl）の `/clear` は対象外 — 後継は別の段階を始めず、ユーザーに確認する。Orca の外では従来どおり。
...
- user スコープでインストールすると、コンテキスト閾値の Stop フックが**すべてのプロジェクト**で動作する。ナッジのメッセージは `.remember/remember.md` へのハンドオフ作成を案内し、**Orca ターミナルではさらに、ナッジ後に後継ターミナルの作成・`.remember/successor-<トークン>.prompt` ファイルの書き込み・旧セッションの終了（`/exit`）が自動で起きる** — `.remember/` を使わないプロジェクトでもそのディレクトリが作られる（プロンプトファイルは送信が確認されれば削除し、引き継ぎが失敗した場合のみ残る）。Orca の外では案内文が出るだけで何も自動実行されない（従来どおり）。
- Orca のターミナルハンドルは環境変数なので、そのターミナル内で起動したすべてのプロセスが引き継ぐ。Orca タブ内で起動した入れ子の `claude`（Bash から実行した `claude -p`、tmux 内の claude など）がナッジを受けると親タブを自分のターミナルとみなし、その後継が親セッションに `/exit` を送る。Orca タブ内で長時間の入れ子 `claude` は動かさず、Orca の外で動かす。
