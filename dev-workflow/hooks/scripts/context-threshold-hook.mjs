...
function orcaHandoff(h) {
  ...
  return (
    ...
    `출력이 GATE_ON이면 자동 인계를 하지 않고 [폴백](그 게이트는 stop_hook_active를 보지 않고 동기 codex 작업 뒤 block을 돌려줄 수 있어 후계의 /exit가 그 턴을 끊습니다). STATE_UNREADABLE(읽기·파싱 실패 — 부재(ENOENT)만 잡 없음이고 권한·경로 오류와 CLAUDE_PLUGIN_DATA 부재는 차단)·RESOLVE_FAIL도 [폴백]. 파일 부재 = GATE_OFF·잡 없음. GATE_OFF일 때만 (2-1)로 진행합니다. ` +
    ...
    `생성: "$CLI" terminal create --worktree active --title "$TITLE" --command "CLAUDE_CODE_DISABLE_TERMINAL_TITLE=1 claude" --json(환경변수는 후계 claude가 탭 제목을 덮어쓰지 않게 해 아래 title 정확 조회를 살린다 — 빼지 마세요) → 새 핸들 = result.startupTerminal.handle(없으면 result.terminal.handle). 응답·JSON이 유실되면 재생성하지 말고 "$CLI" terminal list --worktree active --json 에서 title이 정확히 "$TITLE"인 항목으로 핸들을 회수합니다 — 정확히 1개가 아니면(0 또는 2+) [폴백](차단 보고). ` +
