...
function orcaHandoff(h) {
  ...
  return (
    ...
    `생성(명령 속 환경변수는 후계 claude가 탭 제목을 덮어쓰지 않게 해 아래 title 정확 조회를 살린다 — 빼지 마세요): "$CLI" terminal create --worktree active --title "$TITLE" --command "CLAUDE_CODE_DISABLE_TERMINAL_TITLE=1 claude" --json → 새 핸들 = result.startupTerminal.handle(없으면 result.terminal.handle). 응답·JSON이 유실되면 재생성하지 말고 "$CLI" terminal list --worktree active --json 에서 title이 정확히 "$TITLE"인 항목으로 핸들을 회수합니다 — 정확히 1개가 아니면(0 또는 2+) [폴백](차단 보고). ` +
