...
// 사전 조건이 우회된다(plan R2-2). CLAUDE_PLUGIN_DATA가 비어 있으면 companion은 상태 파일을 os.tmpdir()/codex-companion 아래로
// 해소해 실제 상태를 보지 못한 채 ENOENT → GATE_OFF가 되므로, 그 변수 부재도 STATE_UNREADABLE(exit 2)이다(impl R3-1).
...
const STATE_PROBE_CMD =
  ...
  `if(!process.env.CLAUDE_PLUGIN_DATA){console.log("STATE_UNREADABLE");process.exit(2)}` +
...
// 후계는 claude 그대로 띄운다(D2) — 오르카 tui-idle은 claude가 쓰는 탭 제목으로 idle을 판정하므로 제목 쓰기를 막으면
// (2-2)가 항상 시간 초과다(1.0.1 CLAUDE_CODE_DISABLE_TERMINAL_TITLE 회귀, AC6 2회차). 탭 제목은 셸 PS1·claude가 덮어쓰므로
// create 응답 유실 시 제목으로 후계를 찾지 않는다. 생성 전후 핸들 집합 차분은 사용자 보고용 후보일 뿐 — 오르카에는 생성 요청과
// 터미널을 묶는 불변 식별자가 없어 후보가 1개여도 그 사이 열린 무관한 탭일 수 있다(impl G1-1) → 채택하지 않고 차단·보고.
function orcaHandoff(h) {
  const successorPrompt =
    ...
    `출력의 FOREIGN_ACTIVE가 0이 아니면 15초 간격으로 다시 세어 0이 될 때까지 기다린다(상한 10분 = 40회). 상한에 닿으면 사용자에게 보고하고 /exit와 codex 시작을 모두 보류하라(보고에는 같은 상태 파일의 해당 queued/running 잡 id·status·sessionId를 함께 적어 사용자가 고아 잡인지 판단해 cancel할 수 있게 하라) — 옛 세션의 SessionEnd가 폴더 공유 브로커를 내려 그 잡을 죽인다. STATE_UNREADABLE(읽기·파싱 실패)은 "잡 없음"이 아니라 차단이다 — 사용자에게 보고하고 보류하라. RESOLVE_FAIL도 보고·보류. 파일 부재 = 잡 없음(GATE_OFF FOREIGN_ACTIVE=0). ` +
  ...
  return (
    ...
    `출력이 GATE_ON이면 자동 인계를 하지 않고 [폴백](그 게이트는 stop_hook_active를 보지 않고 동기 codex 작업 뒤 block을 돌려줄 수 있어 후계의 /exit가 그 턴을 끊습니다). STATE_UNREADABLE(읽기·파싱 실패 — 부재(ENOENT)만 잡 없음이고 권한·경로 오류와 CLAUDE_PLUGIN_DATA 부재는 차단)·RESOLVE_FAIL도 [폴백]. 파일 부재 = GATE_OFF·잡 없음. GATE_OFF일 때만 ⓓ로 진행합니다. ` +
    `ⓓ 생성 전 목록: (2-1) create 직전에 "$CLI" terminal list --worktree active --json 을 실행해 result.terminals[].handle 전부를 기록해 둡니다(create 응답 유실 시 사용자에게 보고할 후보 산출 기준 — 탭 제목은 셸 프롬프트와 claude가 곧 덮어쓰므로 핸들 식별에 쓰지 않습니다). ok가 true가 아니거나 result.truncated가 false가 아니면 [폴백]. ` +
    ...
    `생성: "$CLI" terminal create --worktree active --title "$TITLE" --command claude --json → 새 핸들 = result.startupTerminal.handle(없으면 result.terminal.handle). 제목은 탭 표시용입니다. 응답·JSON이 유실되면 재생성하지 말고 "$CLI" terminal list --worktree active --json 을 다시 실행해 ⓓ에서 기록한 핸들 집합에 없는 새 핸들을 후보로 모읍니다 — 후보가 이 세션이 만든 후계라는 증거가 없으므로(그 사이 사용자나 다른 세션이 연 탭일 수 있습니다) 후보가 1개여도 채택하지 않습니다: 후보에 프롬프트 전송·/exit·close를 하지 말고 [폴백] (d)로 갑니다. ` +
    ...
    `영수증 result.send.prompt.stages에 turn_started가 있어야 인계 확정입니다(accepted:true는 입력 수락일 뿐 턴 시작 증명이 아닙니다). input_accepted에서 멈추면 재전송하지 말고 같은 영수증의 result.send.prompt.requestId로 같은 명령에 --wait-submit 30 --retry-request <id>를 붙여 1회 재관찰합니다. 응답 유실 등 모호한 전송 오류도 같은 --retry-request로 재조정합니다. 재관찰 뒤에도 turn_started가 없으면 [폴백]. turn_started를 확인하면 프롬프트 파일을 지웁니다: rm -f ".remember/successor-$TOKEN.prompt"([폴백]이면 진단용으로 남깁니다). ` +
    ...
    `[폴백] 조건 4종: (a) (2-0) 사전 검증 실패 · (b) (2-2) wait satisfied:false(재시도 후) · (c) (2-3) turn_started 미확인(재관찰 후) · (d) CLI 실행 오류(재조정 후 — create 응답 유실은 회수하지 않습니다 — 생성 전후 핸들 집합 차분으로 후보만 산출해, 재생성과 /clear 안내를 모두 차단하고 후보 핸들(목록 조회 실패·truncated면 그 사실)을 사용자에게 보고한 뒤 정지합니다). ` +
    `(2-0)에서 [폴백]하면(사전 검증 실패·GATE_ON·STATE_UNREADABLE·RESOLVE_FAIL·ⓓ 목록 조회 실패) 후계를 만들기 전이라 정리할 터미널이 없으므로, 아래 정리·소멸 확인 없이 바로 사용자에게 "이어서 진행하려면 /clear 후 같은 작업을 다시 시작하세요"라고 안내하세요. 후계 생성((2-1) create)을 시도한 뒤의 [폴백]은 아래를 따릅니다(create 응답 유실은 예외 — 새 핸들을 모르므로 아래 정리 없이 (d)대로 보고하고 정지합니다). 미전달이 확정되기 전(재관찰·재조정 중)에는 후계를 닫지 마세요 — 후계가 이미 프롬프트를 받아 이 세션에 /exit를 보내는 중일 수 있습니다. 확정되면 정리 전에 공유 브로커 잡 검사를 먼저 합니다 — 후계의 /exit와 그 뒤 사용자의 /clear는 둘 다 SessionEnd로 이 폴더의 공유 codex 브로커를 내려 다른 세션의 잡을 죽입니다: ${COMPANION_ROOT_CMD}; ${STATE_PROBE_CMD}. FOREIGN_ACTIVE가 0이 아니면 15초 간격으로 다시 세어 0이 될 때까지 기다립니다(상한 10분 = 40회). 상한에 닿거나 STATE_UNREADABLE·RESOLVE_FAIL이면 후계 /exit·close·/clear 안내를 모두 보류하고 차단 상태(후계 핸들·원인 — 상한이면 해당 queued/running 잡 id·status·sessionId)를 사용자에게 보고한 뒤 정지하세요. FOREIGN_ACTIVE=0이면 만들다 만 후계 터미널을 이 세션이 먼저 정리합니다:claude가 뜬 이력이 있으면(wait satisfied:true였거나 "$CLI" terminal read --terminal "<새 핸들>" --json 에 claude 프롬프트가 보이면) "$CLI" terminal send --terminal "<새 핸들>" --text "/exit" --enter --json → "$CLI" terminal wait --terminal "<새 핸들>" --for exit --timeout-ms 30000 --json(시간 초과가 보통이다 — 그때 "$CLI" terminal read --terminal "<새 핸들>" --json 의 마지막 줄들에서 claude 화면이 사라지고 셸 프롬프트가 복귀했는지 확인. 프롬프트를 받지 못한 빈 후계는 종료 표지("Resume this session with")를 출력하지 않으므로 이 정리에서는 셸 프롬프트 복귀만으로 충분하다 — 옛 세션 종료(첫 동작 ②)의 조건은 그대로. tui-idle 불인정) →"$CLI" terminal close --terminal "<새 핸들>" --json; 뜨지 않았으면 close 직접. /exit 처리 증거 없이 close하지 마세요(증거가 없으면 사용자에게 보고). ` +
