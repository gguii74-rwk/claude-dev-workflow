...
function orcaHandoff(h) {
  ...
  return (
    ...
    `(2-0)에서 [폴백]하면(사전 검증 실패·GATE_ON·STATE_UNREADABLE·RESOLVE_FAIL) 후계를 만들기 전이라 정리할 터미널이 없으므로, 아래 정리·소멸 확인 없이 바로 사용자에게 "이어서 진행하려면 /clear 후 같은 작업을 다시 시작하세요"라고 안내하세요. 후계 생성((2-1) create)을 시도한 뒤의 [폴백]은 아래를 따릅니다. 미전달이 확정되기 전(재관찰·재조정 중)에는 후계를 닫지 마세요 — 후계가 이미 프롬프트를 받아 이 세션에 /exit를 보내는 중일 수 있습니다. 확정되면 정리 전에 공유 브로커 잡 검사를 먼저 합니다 — 후계의 /exit와 그 뒤 사용자의 /clear는 둘 다 SessionEnd로 이 폴더의 공유 codex 브로커를 내려 다른 세션의 잡을 죽입니다: ${COMPANION_ROOT_CMD}; ${STATE_PROBE_CMD}. FOREIGN_ACTIVE가 0이 아니면 15초 간격으로 다시 세어 0이 될 때까지 기다립니다(상한 10분 = 40회). 상한에 닿거나 STATE_UNREADABLE·RESOLVE_FAIL이면 후계 /exit·close·/clear 안내를 모두 보류하고 차단 상태(후계 핸들·원인)를 사용자에게 보고한 뒤 정지하세요. FOREIGN_ACTIVE=0이면 만들다 만 후계 터미널을 이 세션이 먼저 정리합니다:claude가 뜬 이력이 있으면(wait satisfied:true였거나 "$CLI" terminal read --terminal "<새 핸들>" --json 에 claude 프롬프트가 보이면) "$CLI" terminal send --terminal "<새 핸들>" --text "/exit" --enter --json → "$CLI" terminal wait --terminal "<새 핸들>" --for exit --timeout-ms 30000 --json(또는 "$CLI" terminal read --terminal "<새 핸들>" --json 의 종료 표지 + 셸 프롬프트 — tui-idle 불인정) → "$CLI" terminal close --terminal "<새 핸들>" --json; 뜨지 않았으면 close 직접. /exit 처리 증거 없이 close하지 마세요(증거가 없으면 사용자에게 보고). ` +
