...
- 구현이 완료됐는데 `## 적대검증 ledger (impl)`이 없거나 미종결이면 **9가 아니라 8**이다. plan을 생략한 트랙은 **spec 문서 말미**에서 그 ledger를 찾는다(plan 파일 부재를 ledger 부재로 오인하지 않는다). 단 **4·5·6 중 종결도 유효한 생략 기록도 없는 단계가 남아 있으면(승격으로 철회된 경우 포함) 그 단계가 8보다 먼저다** — 구현 완료가 무기록 생략을 사면하지 않는다.
