...
| 3.5 | spec `## UI 설계` 섹션 + 적힌 목업 경로 실재(`docs/design/` 존재만으로 완료 아님, 불일치는 ui-mockup 재개 경로로) | 비대상(화면을 만들거나 바꾸지 않음 · 문구·색·아이콘·단일 컨트롤 수준 경미 변경) 또는 유효 생략 기록(D번호+근거) | ui-mockup |
...
1. **fail-closed** — 신호가 불명확하면(6항목 내용 불명 · harden 실행 불명 = D블록이 비었거나 결정·근거 결여(결정·근거가 실재하는 D블록의 커밋 이력은 재감사하지 않는다) · repo 밖 사실 · 승격 사유와 생략 기록의 공존) 통과시키지 않고 **사용자 1회 확인** — 확인 전 기본값은 정식(그 단계 미완 취급).
