...
- **판정 = 지도 1차 + 사용자 확인 1회.** 접촉 표면 신호로 1차 판정하고 확인을 받는다. **승인했을 때만 경량으로 진입한다** — 거절·무응답·모호한 답은 승인이 아니므로 **정식을 유지**한다. 승인은 **spec의 재논의 금지 블록에 D번호로 남긴다**(생략 기록과 같은 자리·형식) — dev-cycle은 남기라고 안내만 하고 직접 쓰지 않는다.
...
- **승격은 즉시** — 진행 중 위 접촉 표면이 새로 드러나면 그 자리에서 정식으로 올리고 사유를 spec에 적는다. 아직 수행하지 않은 단계의 생략 기록과 **경량 승인 기록을 그때 함께 철회**한다(②는 철회된 기록을 인정하지 않으므로, 재개해도 승인이 승격을 덮지 못한다). task 증식·라운드 초과 같은 진행 신호는 승격 트리거가 아니다.
...
**② 생략 기록 대조** — 산출물 부재(ledger 없음·plan 없음)를 미완으로 읽기 **전에** 재논의 금지 블록의 생략 기록을 본다. 기록이 있으면 그 단계는 미완이 아니라 **해당 없음**이므로 되돌리지 않고 다음으로 보낸다. 단 **기록 없는 부재는 종전대로 미완**이고(경량이었다고 사후 추정하지 않는다), **철회된 기록은 인정하지 않는다.** **경량 승인 기록도 여기서 함께 읽는다** — 있으면 재확인 없이 그 경로를 잇고, 없으면 승인을 추정하지 말고 §경량 경로의 확인을 다시 거친다.
