...
- **가드를 focus text로도 전달한다(이중 방어).** 문서 내 가드만으로는 불완전하다(실측: 가드 블록·ledger가 diff 안에 있어도 동일 항목이 5라운드 연속 재지목). **매 적대 라운드 직전에 재조립**해 리뷰 커맨드에 부착한다(§2b) — 직전 라운드에 닫힌 항목이 즉시 반영된다. 조립 결과는 상태로 보관하지 않는다(원본이 커밋된 문서라 resume 후 동일 재구성 — 핸드오프 필드 없음). 문서 내 가드와 §2c 사후 대조는 그대로 유지한다 — focus는 가중치일 뿐 보장이 아니다.
  ...
  - **문구 = 무효 선언형 + 정상보고 단서 짝 필수**: "아래는 이미 판정으로 닫힌 항목이다. 다시 보고하면 그 finding은 무효로 각하된다(근거는 각 항목에 있다)" + **"이 목록에 없는 문제는 평소대로 전부 보고하라"**. 후자를 빼지 않는다 — 기저 리뷰 템플릿의 스탠스("후속 예정을 인정하지 말라")를 단순 사실 고지로는 못 이기고, 단서 없는 무효 선언은 오억제로 기운다.
...
- **focus의 정상보고 단서를 빼지 말 것** — "이 목록에 없는 문제는 평소대로 전부 보고하라"가 없는 무효 선언은 오억제로 기운다(짝 필수 — §기결정 가드).
