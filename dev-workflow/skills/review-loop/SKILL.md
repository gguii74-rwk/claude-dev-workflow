...
- **확인 예산 소진(복귀 발동 여부 무관) + 미확인 FIXED 큐 잔존** → ESCALATE + 폴백 3택을 ledger에 명시: ① 다음 phase 리뷰의 필수 확인 항목으로 이월, ② 새 세션에서 확인 라운드 1회(신규 결함이 나오면 whole-branch 리뷰 권고), ③ 배포 smoke/검증을 blocking 체크로 대체.
  - **폴백②는 확인 예산 밖 예약분이다.** 사용자가 ②를 고르면 **§2i의 일시중단 절차**로 상태를 남기고(그러지 않으면 재개할 파일이 없다), 복원 필드 `폴백② 허용·소비 여부`를 *허용·미소비*로 적는다. 재개 세션은 그 값이 *허용·미소비*이면 **확인 예산 잔여 0 가드를 정확히 한 번 우회**한다(§2h ②). 이 우회가 없으면 §2h ②가 같은 ESCALATE로 되돌려 사용자가 승인한 확인 1회를 영원히 못 돈다.
  - **소비 시점** = **완전한 응답을 받았을 때만** *소비*로 바꾼다. 실패·타임아웃·부분 응답이면 **미소비 유지**(§응답 완전성 계약과 동형 — 자동 재시도는 여전히 금지이고 재실행은 사용자 판단이다).
  - **카운터** = 폴백② 라운드는 `확인 소진`을 올리지 않는다(예약분이므로 — §2j). 복귀 재진입 확인이 받는 예외와 같은 취급이다.
  - **재승인 없음** = 소비 후에는 폴백②를 다시 제시하지 않는다. 남은 선택지는 ①·③이다.
...
- **복원 필드**: phase / 적대 라운드 소진 카운트 / 확인 라운드 소진 카운트 / 현재 모드(적대·확인) / 복귀 사용 여부 / **폴백② 허용·소비 여부** / **실행 예산 3종의 해소된 값(`--max`·`--confirm-rounds`·`--auto-rounds`)** / base(해소된 ref + SHA) / branch / 중단 시 HEAD SHA / ledger(미확인 FIXED 큐 명시 포함) / score 이력 / 보안 크리티컬 트랙 판정 상태.
...
- **전환 신호 발화 시**(§blocking score 3종 — 보안 트랙은 신호 2·3만):
  ...
  2. 확인 필수 조건 점검(§확인 모드): 미확인 FIXED 큐도 루프 직접 판정도 0건이면 §2e 빠른 종료. 아니면 **확인 예산 잔여를 먼저 검사한다** — 잔여 0(`--confirm-rounds 0`으로 시작했거나 이미 소진)이면 `task`를 실행하지 않고 **ESCALATE**(§인자·§확인 모드 결과 처리). 새 루프는 시작 시 큐가 비어 §0 검사를 통과하므로, **확인 진입 시점의 이 검사가 `--confirm-rounds 0` 계약이 실제로 걸리는 유일한 지점**이다.
     - **예외 — `폴백② 허용·미소비`면 이 가드를 정확히 한 번 우회한다**(§확인 모드 결과 처리). 사용자가 이미 그 라운드를 승인했는데 잔여 0으로 되돌리면 승인한 확인을 영원히 못 돈다. 완전한 응답을 받으면 *소비*로 표시해 다시 잠근다.
...
- **복귀 후 재진입 확인도 확인 진입이다 — 단 예약분이라 예산 검사는 면제한다.** 복귀 적대 라운드는 카운터를 올리지 않아 auto-rounds 경계 이전이면 여전히 자동 모드로 돌고, 그 라운드가 batch-pending을 적재할 수 있다. 따라서 재진입 확인 직전에도 **위 ①(batch 일괄 제시)은 그대로 수행**한다 — "확인 진입 시점에 batch가 비어 있다"는 전제는 최초 진입뿐 아니라 모든 확인 진입 경로에 적용되며, flush가 없으면 적재 항목이 미판정인 채 확인 verdict를 받거나 교착한다.
  - **위 ②(확인 예산 잔여 검사)는 재진입 확인에 적용하지 않는다.** 복귀가 여는 확인 1라운드는 `--confirm-rounds` 상한 밖 예약분이라 잔여가 0이어도 실행한다(§인자·§확인 모드 결과 처리 "복귀가 폴백보다 우선"). 예약분에 일반 예산 검사를 걸면 `--confirm-rounds 1`에서 C1이 blocking을 낸 순간 예산이 0이 되어 복귀 수정의 필수 재확인이 통째로 차단된다. **예약분 소비 여부는 예산 잔여와 별도 상태로 추적한다** — 복귀는 `복귀 사용 여부`, 폴백②는 `폴백② 허용·소비 여부`(§0·§2i 복원 필드). 폴백②로 재개한 확인 라운드도 같은 이유로 이 검사를 우회한다.
...
| **확인 — 예약분**(복귀 재진입 · 폴백②) | **`확인 소진` 갱신 없음** — 폴백②는 완전한 응답을 받았을 때 `폴백② 소비`로만 표시한다 | 상한 밖이라 예산 잔여와 무관하게 실행된다(§확인 모드) |
| **확인** (실패·타임아웃·부분 응답) | **갱신 없음** — 예약분이면 **미소비 유지** | 중단 보고(§응답 완전성 계약) |
...
- **복귀로 추가되는 적대 1라운드와 재진입 확인 1라운드, 폴백②의 확인 1라운드는 각 상한 밖**이며 두 카운터 어느 쪽도 올리지 않는다(§인자 `--confirm-rounds` 산법). 예약분의 소비 여부는 예산 잔여와 **별도 상태**로 추적한다 — 복귀는 `복귀 사용 여부`, 폴백②는 `폴백② 허용·소비 여부`(§0·§2i 복원 필드).
...
- ⏸ ESCALATE에서 사용자가 중단 선택 → **§2i 일시중단**(루프 파일 유지)
- ⏸ 컨텍스트 ≥40% → **§2i 일시중단** + /clear 안내 후 종료(resume로 이어감)
- ⏸ 폴백②(새 세션 확인 1회) → **§2i 일시중단** + `폴백② 허용·미소비` 기록(예산 밖 예약분)
- 🗂 **루프 파일 수명**: 재개하는 3경로(위 ⏸ 세 줄)는 **유지**, 종결하는 3경로(성공 종료 · 폴백① · 폴백③)는 §4의 인계를 마친 뒤 **제거**. 남아 있는 `loop-*.md`는 항상 재개 대기 중인 루프의 것이다
...
- **저장소 유래 문자열을 셸 커맨드에 직접 보간하지 말 것** — 확인 프롬프트뿐 아니라 **적대 가드 focus 경로에도** 걸린다. 파일에 쓴 뒤 `"$(cat <파일>)"`로 읽어 `--` 뒤 인자 하나로 넘긴다(§2b·§확인 모드).
- **focus의 정상보고 단서를 빼거나 한 줄로 줄이지 말 것** — 단서 없는 무효 선언은 오억제로 기울고, 한 줄 단서도 부족함이 실측됐다(강화형 3항이 규격 — §기결정 가드).
- **루프 진행 중에 `.remember/remember.md`를 건드리지 말 것** — 루프 상태는 `loop-<ledger basename>-<phase>.md`에 쓴다. 공유 파일에 쓰면 다른 트랙 인계를 덮어쓴다(§2i).
- **종결 인계에서 기존 `remember.md`를 확인 없이 덮어쓰지 말 것** — 먼저 읽고, 이번 루프가 갱신하는 부분 외 섹션은 보존한다(§4).
- **일시중단 시 커밋 전 HEAD를 루프 파일에 적지 말 것** — 뒤따르는 커밋이 그 값을 무효화해 재개가 항상 §0에서 막힌다(§2i 순서 1→2).
- **미확인 FIXED 큐의 개별 항목을 적대 focus에 넣지 말 것**(닫힌 항목 목록으로든 '집중 검증' 요청으로든) — 소멸 확인은 확인 라운드 임무 ①이고, 적대 비재출현은 소멸 증거가 아니다(§두 큐). focus에 허용되는 큐 언급은 진행 상태 한 줄뿐이다(§기결정 가드).
