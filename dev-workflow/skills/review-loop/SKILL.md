...
- **복귀 후 재진입 확인도 확인 진입이다.** 복귀 적대 라운드는 카운터를 올리지 않아 auto-rounds 경계 이전이면 여전히 자동 모드로 돌고, 그 라운드가 batch-pending을 적재할 수 있다. **재진입 확인 직전에도 위 ①(batch 일괄 제시)·②(예산 검사)를 그대로 수행한다** — "확인 진입 시점에 batch가 비어 있다"는 전제는 최초 진입뿐 아니라 모든 확인 진입 경로에 적용된다. 이 flush가 없으면 적재된 항목이 미판정인 채로 확인 verdict를 받거나 flush 지점 없이 교착한다.
